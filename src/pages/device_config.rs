...
use crate::pages::wifi_provision;
use crate::services::ble::modbus::{
    bind_option_standalone_only, bind_trigger_field, builtin_bind_supported,
    encode_write_value, enum_index_for_value, integer_debug_hex, is_parallel_ha1_device,
    is_wifi_poll_index, parse_register_address, parse_value_type, BuiltinSettingDef, BuiltinWidget,
...
use crate::state::device_config::custom_groups_to_schema;
...
pub fn refresh_builtin_availability(ui: &MainWindow, ctx: &AppContext) {
    ...
    wifi_provision::refresh_wifi_status(ui, ctx);
}
...
fn persist_device_config(ctx: &AppContext, ui: &MainWindow) {
    ...
    let custom = custom_groups_to_schema(&groups);
    if let Err(e) = device_config_store::save(&custom, active, groups.row_count()) {
        warn!(
    ...
    if builtin {
        for r in &snapshot.items {
            if is_wifi_poll_index(r.item_index) {
                wifi_provision::apply_wifi_poll_item(ctx, r);
                continue;
            }
            let st = ctx.state.borrow();
        ...
        wifi_provision::advance_wifi_phase(ctx);
        wifi_provision::refresh_wifi_status(ui, ctx);
        return;
    }
...
pub fn wire(ui: &MainWindow, ctx: &AppContext) {
    ...
    wifi_provision::wire_wifi(ui, ctx);
}
