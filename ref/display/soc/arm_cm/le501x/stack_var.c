...
#else
#define DB_BUF_SIZE (1024 * 6)
#define MSG_BUF_SIZE (1024 * 6)
...
__attribute((weak)) void (*ecc_calc_fn)(
    const uint8_t *,
    const uint8_t *,
    const uint8_t *,
    uint8_t *,
...
uint32_t heap_buf
    [ENV_BUF_SIZE / sizeof(uint32_t) + DB_BUF_SIZE / sizeof(uint32_t)
     + MSG_BUF_SIZE / sizeof(uint32_t) + NON_RET_BUF_SIZE / sizeof(uint32_t)];

__attribute((weak)) void statck_buffer_init(
...
__attribute((weak)
) void (*eif_read)(uint8_t *bufptr, uint32_t size, void (*callback)(void *, uint8_t), void *dummy);
__attribute((weak)
) void (*eif_write)(uint8_t *bufptr, uint32_t size, void (*callback)(void *, uint8_t), void *dummy);
__attribute((weak)) void (*eif_flow_on)(void);
__attribute((weak)) bool (*eif_flow_off)(void);

__attribute__((weak)) void uart_eif_read(
    uint8_t *bufptr, uint32_t size, void (*callback)(void *, uint8_t), void *dummy
) {
}
__attribute__((weak)) void uart_eif_write(
    uint8_t *bufptr, uint32_t size, void (*callback)(void *, uint8_t), void *dummy
) {
}
__attribute__((weak)) void uart_eif_flow_on(void) {
}
__attribute__((weak)) bool uart_eif_flow_off(void) {
    return false;
}
...
__attribute__((weak)) void ll_stack_buffer_init(
    uint32_t env_size, uint32_t msg_size, uint32_t non_ret_size
) {};
