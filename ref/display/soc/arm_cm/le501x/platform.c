...
#define IRQ_NVIC_PRIO(IRQn, priority) \
...
static void irq_priority() {
    ...
    NVIC->IP[0] = IRQ_NVIC_PRIO(EXTI_IRQn, 3) | IRQ_NVIC_PRIO(WWDT_IRQn, 3)
                  | IRQ_NVIC_PRIO(LPWKUP_IRQn, 2) | IRQ_NVIC_PRIO(BLE_IRQn, 1);
    NVIC->IP[1] = IRQ_NVIC_PRIO(RTC_IRQn, 3) | IRQ_NVIC_PRIO(DMA_IRQn, 3)
                  | IRQ_NVIC_PRIO(QSPI_IRQn, 2) | IRQ_NVIC_PRIO(ECC_IRQn, 3);
    NVIC->IP[2] = IRQ_NVIC_PRIO(CACHE_IRQn, 3) | IRQ_NVIC_PRIO(TRNG_IRQn, 3)
                  | IRQ_NVIC_PRIO(IWDT_IRQn, 3) | IRQ_NVIC_PRIO(CRYPT_IRQn, 3);
    NVIC->IP[3] = IRQ_NVIC_PRIO(PDM_IRQn, 3) | IRQ_NVIC_PRIO(BLE_WKUP_IRQn, 1)
                  | IRQ_NVIC_PRIO(ADC_IRQn, 3) | IRQ_NVIC_PRIO(ADTIM1_IRQn, 3);
    NVIC->IP[4] = IRQ_NVIC_PRIO(BSTIM1_IRQn, 3) | IRQ_NVIC_PRIO(GPTIMA1_IRQn, 3)
                  | IRQ_NVIC_PRIO(GPTIMB1_IRQn, 3) | IRQ_NVIC_PRIO(BLE_ERR_IRQn, 1);
    NVIC->IP[5] = IRQ_NVIC_PRIO(LVD33_IRQn, 0) | IRQ_NVIC_PRIO(GPTIMC1_IRQn, 3)
                  | IRQ_NVIC_PRIO(LPTIM_IRQn, 3) | IRQ_NVIC_PRIO(I2C1_IRQn, 3);
    NVIC->IP[6] = IRQ_NVIC_PRIO(I2C2_IRQn, 3) | IRQ_NVIC_PRIO(SPI1_IRQn, 3)
                  | IRQ_NVIC_PRIO(SPI2_IRQn, 3) | IRQ_NVIC_PRIO(UART1_IRQn, 3);
    NVIC->IP[7] = IRQ_NVIC_PRIO(UART2_IRQn, 3) | IRQ_NVIC_PRIO(UART3_IRQn, 3)
                  | IRQ_NVIC_PRIO(BLE_FIFO_IRQn, 1) | IRQ_NVIC_PRIO(BLE_CRYPT_IRQn, 1);
}
...
void mesh_stack_data_bss_init(void) {
    ...
    memcpy(
        &__mesh_stack_data_start__, &__mesh_stack_data_lma__, (uint32_t)&__mesh_stack_data_size__
    );
}

static void ble_irq_clr() {
    // clear ble irq
    NVIC->ICPR[0] = 1 << BLE_IRQn | 1 << BLE_ERR_IRQn | 1 << BLE_FIFO_IRQn
                    | (unsigned int)1 << BLE_CRYPT_IRQn;
}

static void ble_irq_enable() {
    // enable ble irq
    NVIC->ISER[0] = 1 << BLE_IRQn | 1 << BLE_ERR_IRQn | 1 << BLE_FIFO_IRQn
                    | (unsigned int)1 << BLE_CRYPT_IRQn;
}
...
void irq_reinit() {
    irq_priority();
    NVIC->ISER[0]
        = 1 << QSPI_IRQn | 1 << CACHE_IRQn | 1 << LPWKUP_IRQn | 1 << EXTI_IRQn | 1 << RTC_IRQn;
    lvd33_irq_enable();
}
...
static void mac_init() {
    ...
    RCC->BLECFG = mac_clk << RCC_BLE_CK_SEL_POS | 1 << RCC_BLE_MRST_POS | 1 << RCC_BLE_CRYPT_RST_POS
                  | 1 << RCC_BLE_LCK_RST_POS | 1 << RCC_BLE_AHB_RST_POS | 1 << RCC_BLE_WKUP_RST_POS
                  | 1 << RCC_BLE_LPWR_CKEN_POS | 1 << RCC_BLE_AHBEN_POS
                  | 1 << RCC_BLE_MDM_REFCLK_CKEN_POS | !SDK_LSI_USED << RCC_BLE_LCK_SEL_POS;
    RCC->BLECFG &= ~(
        1 << RCC_BLE_MRST_POS | 1 << RCC_BLE_CRYPT_RST_POS | 1 << RCC_BLE_LCK_RST_POS
        | 1 << RCC_BLE_AHB_RST_POS | 1 << RCC_BLE_WKUP_RST_POS
    );
}
...
__attribute__((weak)) void *builtin_timer_create(void (*cb)(void *)) {
    return NULL;
}
...
static void lsi_calib() {
    ...
    MODIFY_REG(
        SYSCFG->ANACFG1,
        SYSCFG_RCO_MODE_SEL_MASK | SYSCFG_RCO_CAL_START_MASK | SYSCFG_EN_RCO_DIG_PWR_MASK,
...
__attribute__((weak)) uint32_t ble_isr() {
    return 0;
}

__attribute__((weak)) void ble_stack_isr() {
    ble_isr();
}
    ...
    if (flash_writing_status) {
        ...
        do {
            hal_flash_read_status_register_1_ram(&status_reg1);
                 ...
                 && (status_reg1 & (STATUS_REG1_SUS1_MASK | STATUS_REG1_SUS2_MASK)) == 0);
    }
    if (xip == false) {
        hal_flash_xip_start();
    }
...
void XIP_BANNED_FUNC(switch_to_rc32k, ) {
    MODIFY_REG(
        RCC->CFG,
        RCC_SYSCLK_SW_MASK | RCC_HCLK_SCAL_MASK | RCC_CKCFG_MASK,
        (uint32_t)2 << RCC_SYSCLK_SW_POS | (uint32_t)1 << RCC_CKCFG_POS
    );
}
...
void XIP_BANNED_FUNC(switch_to_xo16m, ) {
    MODIFY_REG(
        RCC->CFG,
        RCC_HCLK_SCAL_MASK | RCC_CKCFG_MASK | RCC_SYSCLK_SW_MASK,
        (uint32_t)1 << RCC_CKCFG_POS | (uint32_t)1 << RCC_SYSCLK_SW_POS
    );
}
...
static void XIP_BANNED_FUNC(switch_to_pll, uint8_t hclk_scal) {
    MODIFY_REG(
        RCC->CFG,
        RCC_SYSCLK_SW_MASK | RCC_HCLK_SCAL_MASK | RCC_CKCFG_MASK,
        (uint32_t)4 << RCC_SYSCLK_SW_POS | (uint32_t)1 << RCC_CKCFG_POS
            | (uint32_t)hclk_scal << RCC_HCLK_SCAL_POS
    );
}
