...
uint8_t Nr; // Number of rounds, which is a function of  Nk  and  Nb (which is fixed). For this
...
// 加密函数
void cipher(uint8_t *in, uint8_t *out, uint8_t *key) {
    uint8_t state[4 * Nb];
    uint8_t r, i, j;
    uint8_t *w;
    ...
    w = malloc(Nb * (Nr + 1) * 4); // 申请扩展密钥长度为176
    key_expansion(key, w); // 扩展密钥
    for (i = 0; i < 4; i++) {
        for (j = 0; j < Nb; j++) {
    ...
    add_round_key(state, w, 0); // 轮密钥加

    for (r = 1; r < Nr; r++) {
        sub_bytes(state); // 字节代替
        shift_rows(state); // 行移位
        mix_columns(state); // 列混合
        add_round_key(state, w, r); // 轮密钥加
    }
    // 最后一轮只执行3个阶段
    sub_bytes(state);
    shift_rows(state);
...
// 解密函数
void inv_cipher(uint8_t *in, uint8_t *out, uint8_t *key) {
    uint8_t state[4 * Nb];
    uint8_t r, i, j;
    uint8_t *w;
    ...
    add_round_key(state, w, Nr); // 轮密钥加

    for (r = Nr - 1; r >= 1; r--) {
        inv_shift_rows(state); // 逆行移位
        inv_sub_bytes(state); // 逆字节代替
        add_round_key(state, w, r); // 轮密钥加
        inv_mix_columns(state); // 逆列混合
    }
