...
#define MDNS_NAME_MAX_LEN \
    (64 + 4) // Need to account for IPv6 reverse queries (64 char address  + ".ip6" )
#else
#define MDNS_NAME_MAX_LEN 64 // Maximum string length of hostname, instance, service and proto
#endif
#define MDNS_NAME_BUF_LEN \
    (MDNS_NAME_MAX_LEN + 1) // Maximum char buffer size to hold hostname, instance, service or proto
 ...
 *     - ESP_FAIL failed to add service
 */
esp_err_t mdns_service_add_for_host(const char *instance_name,
                                    const char *service_type,
                                    const char *proto,
                                    const char *hostname,
                                    uint16_t port,
 ...
 *     - false  Service not found.
 */
bool mdns_service_exists_with_instance(const char *instance,
                                       const char *service_type,
                                       const char *proto,
                                       const char *hostname);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_remove_for_host(const char *instance,
                                       const char *service_type,
                                       const char *proto,
                                       const char *hostname);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_instance_name_set_for_host(const char *instance_old,
                                                  const char *service_type,
                                                  const char *proto,
                                                  const char *hostname,
                                                  const char *instance_name);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_port_set_for_host(const char *instance,
                                         const char *service_type,
                                         const char *proto,
                                         const char *hostname,
                                         uint16_t port);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_set(const char *service_type,
                               const char *proto,
                               mdns_txt_item_t txt[],
                               uint8_t num_items);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_set_for_host(const char *instance,
                                        const char *service_type,
                                        const char *proto,
                                        const char *hostname,
                                        mdns_txt_item_t txt[],
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_item_set(const char *service_type,
                                    const char *proto,
                                    const char *key,
                                    const char *value);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_item_set_with_explicit_value_len(const char *service_type,
                                                            const char *proto,
                                                            const char *key,
                                                            const char *value,
                                                            uint8_t value_len);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_item_set_for_host(const char *instance,
                                             const char *service_type,
                                             const char *proto,
                                             const char *hostname,
                                             const char *key,
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_item_set_for_host_with_explicit_value_len(const char *instance,
                                                                     const char *service_type,
                                                                     const char *proto,
                                                                     const char *hostname,
                                                                     const char *key,
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_item_remove(const char *service_type,
                                       const char *proto,
                                       const char *key);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_txt_item_remove_for_host(const char *instance,
                                                const char *service_type,
                                                const char *proto,
                                                const char *hostname,
                                                const char *key);
 ...
 *
 * @param  instance_name    instance name. If NULL, will find the first service with the same
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_subtype_add_for_host(const char *instance_name,
                                            const char *service_type,
                                            const char *proto,
                                            const char *hostname,
                                            const char *subtype);
 ...
 *     - ESP_ERR_NOT_FOUND Service not found
 */
esp_err_t mdns_service_subtype_remove_for_host(const char *instance_name,
                                               const char *service_type,
                                               const char *proto,
                                               const char *hostname,
                                               const char *subtype);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_subtype_add_multiple_items_for_host(const char *instance_name,
                                                           const char *service_type,
                                                           const char *proto,
                                                           const char *hostname,
                                                           mdns_subtype_item_t subtype[],
                                                           uint8_t num_items);
 ...
 *     - ESP_ERR_NO_MEM memory error
 */
esp_err_t mdns_service_subtype_update_multiple_items_for_host(const char *instance_name,
                                                              const char *service_type,
                                                              const char *proto,
                                                              const char *hostname,
                                                              mdns_subtype_item_t subtype[],
 ...
 *        Pointer to search object has to be deleted via `mdns_query_async_delete` once the query
 * has finished. The results although have to be freed manually.
 *
 * @param search pointer to search object
 * @param timeout time in milliseconds to wait for answers
 * @param results pointer to the results of the query
 * @param num_results pointer to the number of the actual result items (set to NULL to ignore this
 * return value)
 *
 * @return
 ...
 */
esp_err_t mdns_query_generic(const char *name,
                             const char *service_type,
                             const char *proto,
                             uint16_t type,
                             mdns_query_transmission_type_t transmission_type,
                             uint32_t timeout,
                             size_t max_results,
                             mdns_result_t **results);
 ...
 *     - ESP_ERR_INVALID_ARG    timeout was not given
 */
esp_err_t mdns_query(const char *name,
                     const char *service_type,
                     const char *proto,
                     uint16_t type,
                     uint32_t timeout,
 ...
 *     - ESP_ERR_INVALID_ARG    parameter error
 */
esp_err_t mdns_query_ptr(const char *service_type,
                         const char *proto,
                         uint32_t timeout,
                         size_t max_results,
                         mdns_result_t **results);
 ...
 *     - ESP_ERR_INVALID_ARG    parameter error
 */
esp_err_t mdns_query_srv(const char *instance_name,
                         const char *service_type,
                         const char *proto,
                         uint32_t timeout,
 ...
 *     - ESP_ERR_INVALID_ARG    parameter error
 */
esp_err_t mdns_query_txt(const char *instance_name,
                         const char *service_type,
                         const char *proto,
                         uint32_t timeout,
                         mdns_result_t **result);
 ...
 *     - ESP_ERR_INVALID_ARG    parameter error
 */
esp_err_t mdns_lookup_delegated_service(const char *instance,
                                        const char *service_type,
                                        const char *proto,
                                        size_t max_results,
                                        mdns_result_t **result);
 ...
 *     - ESP_ERR_INVALID_ARG    parameter error
 */
esp_err_t mdns_lookup_selfhosted_service(const char *instance,
                                         const char *service_type,
                                         const char *proto,
                                         size_t max_results,
                                         mdns_result_t **result);
 ...
 *
 * @param  host_name    host name to look for
 * @param  timeout      time in milliseconds to wait for answer. If 0, max_results needs to be
 * defined
 ...
 *     - ESP_ERR_NO_MEM         not enough memory for this in interface in the netif list (see
...
/**
 * @brief   Set esp_netif to a desired state, or perform a desired action, such as enable/disable
 * this interface or send announcement packets to this netif
 *
 *  * This function is used to enable (probe, resolve conflicts and announce), announce, or disable
 * (send bye) mDNS services on the specified network interface.
 *  * This function must be called if users registers a specific interface using
 * mdns_register_netif() to enable mDNS services on that interface.
 *  * This function could be used in IP/connection event handlers to automatically enable/announce
 * mDNS services when network properties change and/or disable them on disconnection.
 *
 * @param esp_netif  Pointer to esp-netif interface
 ...
 *         NULL otherwise.
 */
mdns_browse_t *mdns_browse_new(const char *service,
                               const char *proto,
 ...
 * @return
 *     - ESP_OK                 success.
 *     - ESP_ERR_FAIL           mDNS is not running or the browsing of `_service._proto` is never
 * started.
 *     - ESP_ERR_NO_MEM         memory error.
 */
