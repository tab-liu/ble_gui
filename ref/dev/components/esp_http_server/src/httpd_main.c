...
static esp_err_t httpd_server_init(struct httpd_data *hd)
{
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &v6only, sizeof(v6only)) < 0) {
        ESP_LOGW(TAG, LOG_FMT("error setting IPV6_V6ONLY (%d)"), errno);
    }

    // 2. 绑定到特定IPv6地址（含IPv4映射）
    struct in6_addr bind_addr = IN6ADDR_ANY_INIT;
    if (inet_pton(PF_INET6, "::FFFF:192.168.8.1", &bind_addr) <= 0) {
        ESP_LOGE(TAG, "Invalid IPv6 address format");
        close(fd);
        return ESP_FAIL;
    }
...
#else
    ip4_addr_t ip_addr;
    IP4_ADDR(&ip_addr, 192, 168, 8, 1);

    struct sockaddr_in serv_addr = {
        .sin_family   = PF_INET,
        .sin_addr     = {
            .s_addr = PP_HTONL(htonl(ip_addr.addr)) //TODO: INADDR_ANY --> webserver临时绑定到Wi-Fi AP上0xC0A80801
        },
