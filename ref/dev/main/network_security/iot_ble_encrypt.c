...
uint8_t md5_authen_value[16]; //MD5值
uint8_t generate_key[16]; //新key
//通信AES秘钥
uint8_t *com_aes_key = NULL;
//文件AES秘钥
...
uint8_t ecc_key_agreement_data[128];
...
uint8_t aes_cbc_key[32]; //AES-CBC密钥
...
void iot_encrypt_data(const uint8_t *scr, uint16_t slen, uint8_t *dst, uint16_t *olen)
{
    ...
    ptr = (uint8_t *)heap_caps_malloc(ptr_len* sizeof(uint8_t), MALLOC_CAP_SPIRAM);
    if(ptr == NULL)
    {
        printf("malloc faild");
        return;
...
void iot_decrypt_data(const uint8_t *src, uint16_t slen, uint8_t *dst, uint16_t dlen, uint16_t *olen)
{
    ...
    uint8_t *ptr = NULL;
    ptr = (uint8_t *)heap_caps_malloc(slen * sizeof(uint8_t), MALLOC_CAP_SPIRAM);
    if(ptr == NULL)
    {
        printf("malloc faild");
        return;
...
void generate_com_aes_key(const uint8_t *key_in)
{
    if(com_aes_key == NULL)
    {
        com_aes_key = (uint8_t*)heap_caps_malloc(16, MALLOC_CAP_SPIRAM);
        if(com_aes_key == NULL)
        {
            printf("malloc faild");
            return;
...
void generate_file_aes_key(const uint8_t *key_in)
{
    ...
    {
        file_aes_key = (uint8_t*)heap_caps_malloc(16, MALLOC_CAP_SPIRAM);
        if(file_aes_key == NULL)
        {
            printf("malloc faild");
            return;
...
static void client_public_gen(uint8_t *public_key)
{
    ...
    cli_grp = (mbedtls_ecp_group*)heap_caps_malloc(sizeof(mbedtls_ecp_group), MALLOC_CAP_SPIRAM);
    if(cli_grp == NULL)
    {
    ...
    cli_pri = (mbedtls_mpi*)heap_caps_malloc(sizeof(mbedtls_mpi), MALLOC_CAP_SPIRAM);
    if(cli_pri == NULL)
    {
    ...
    cli_ctr_drbg = (mbedtls_ctr_drbg_context*)heap_caps_malloc(sizeof(mbedtls_ctr_drbg_context), MALLOC_CAP_SPIRAM);
    if(cli_ctr_drbg == NULL)
    {
...
void server_gen_public_key(uint8_t *public_key)
{
    ...
    if(srv_grp == NULL)
    {
        srv_grp = (mbedtls_ecp_group*)heap_caps_malloc(sizeof(mbedtls_ecp_group), MALLOC_CAP_SPIRAM);
        if(srv_grp == NULL)
        {
            printf("malloc fail!");
            goto cleanup;;
    ...
    if(srv_pri == NULL)
    {
        srv_pri = (mbedtls_mpi*)heap_caps_malloc(sizeof(mbedtls_mpi), MALLOC_CAP_SPIRAM);
        if(srv_pri == NULL)
        {
            printf("malloc fail!");
            goto cleanup;;
    ...
    {
        srv_ctr_drbg = (mbedtls_ctr_drbg_context*)heap_caps_malloc(sizeof(mbedtls_ctr_drbg_context), MALLOC_CAP_SPIRAM);
        if(srv_ctr_drbg == NULL)
        {
            printf("malloc fail!");
            goto cleanup;;
...
// void mobile_pack_sign(uint8_t *data, uint16_t len, uint8_t *sign_out)
// {
//     ecdsa_sign(data, len, mobile_sign_private_key, sign_out);
// }

// uint8_t mobile_pack_verify(uint8_t *data, uint16_t len, uint8_t *sign_data)
// {
//     return ecdsa_verify(data, len, sign_data, mobile_sign_public_key);
// }
...
// void mobile_ecdh_key_agreement_data(uint8_t *public_key, uint8_t *out)
// {
//     uint8_t sign_data[64];
//     memcpy(out, public_key, 64);
//     memcpy(out+64, md5_authen_value, 16); //第一次MD5的数据
//     mobile_pack_sign(out, 80, sign_data); //ECDSA签名
//     memcpy(out+64, sign_data, sizeof(sign_data)); 
// }
...
// //密钥协商请求
// uint16_t mobile_ble_authen_ecdh_key_agreement_respond(uint8_t *data)
// {
//     uint16_t len = 0;;
//     uint8_t pack_data[128];
//     uint8_t pub[64];
//     server_gen_public_key(pub); //生成协商密钥
//     //dump_buf("Mobile ECDH PUB:", pub, sizeof(pub));
//     mobile_ecdh_key_agreement_data(pub, pack_data);
//     len = iot_ble_authen_pack(ECDH_AGRESSMENT_RESPOND, pack_data, sizeof(pack_data), data); //打包
//     return len;
// }

//手机检查密钥协商请求包
// uint8_t mobile_ble_authen_ecdh_key_agreement_request_cheak(uint8_t *data, uint8_t len, uint8_t *reslt_resp, uint8_t *rlen, uint8_t *share_key)
// {
//     uint8_t ret = 0;
//     uint8_t sign_data[64];
//     uint8_t verify_data[80];

//     if(!check_pack_format(data, len, ECDH_AGRESSMENT_REQUEST)) //格式检查
//     {
//         printf("mobile format error!\n");
//         return ret;
//     }

//     memcpy(sign_data, data+68, sizeof(sign_data)); //复制签名值
//     //dump_buf("mobile get sign_data:", sign_data, sizeof(sign_data));
//     memcpy(verify_data, data+4, 64); //复制公钥部分
//     memcpy(verify_data+64, md5_authen_value, 16); //第一次MD5的计算结果
//     if(mobile_pack_verify(verify_data, sizeof(verify_data), sign_data)) //验签
//     {
//         server_calculate_secret(verify_data, share_key); //计算ECDH共享密钥
//         //dump_buf("mobile_share_key:", share_key, 32);
...
{
    aes_key_init(); //初始化AES秘钥
//    check_for_cert_update();
    if(load_all_cer())//加载证书和私钥
    {
        ESP_LOGI(TAG, "load cert success!");
        rsa_key_init();//RSA公钥和私钥初始化
    }
    else
    {
        ESP_LOGE(TAG, "load cert fail!");
		zzapp_main();//debug first write
    }
...
//    aes_key_init();
    ...
    ESP_LOGI(TAG, "**********system file list**********");
//    file_list(ota_bin_path); // 显示文件系统文件
    ESP_LOGI(TAG, "**********system file list**********");
