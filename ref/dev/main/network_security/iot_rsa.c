...
{
    entropy_ptr = (mbedtls_entropy_context*)heap_caps_malloc(sizeof(mbedtls_entropy_context)+1, MALLOC_CAP_SPIRAM);
    ctr_drbg_ptr = (mbedtls_ctr_drbg_context*)heap_caps_malloc(sizeof(mbedtls_ctr_drbg_context)+1, MALLOC_CAP_SPIRAM);
...
//	if (0 != mbedtls_pk_parse_key(&ctx_pk_ptr, private_key, strlen((char*)private_key) + 1, NULL, 0,mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE))
    ...
    rsa_ptr = (mbedtls_rsa_context*)heap_caps_malloc(sizeof(mbedtls_rsa_context), MALLOC_CAP_SPIRAM);
...
void extract_rsa_public_key(uint8_t *cert)
{
    ...
    public_rsa_ptr = (mbedtls_rsa_context*)heap_caps_malloc(sizeof(mbedtls_rsa_context), MALLOC_CAP_SPIRAM);
    ...
    // {
    //     rsa_public_key.key = (uint8_t*)malloc((cacert.pk_raw.len+1)*sizeof(uint8_t));
    //     if(rsa_public_key.key == NULL)
    //     {
    //         mbedtls_printf( "rsa_public_key malloc faild\n");
    //         goto exit;
...
//    ret = mbedtls_rsa_pkcs1_encrypt(rsa_ptr, mbedtls_ctr_drbg_random, ctr_drbg_ptr, MBEDTLS_RSA_PUBLIC, plaintext_len, plaintext, ciphertext);
    ...
    /***** base64编码 *****/
    // b64_out = malloc(sig_len*2);//必须要*2！！！  进行base64解码后的长度会比原来的长1/3
    ...
    // strncpy((char *)ciphertext, (bin_to_hex(sig_buff, (size_t)sig_len)), sig_len);
    /***** 十六进制编码 *****/
    ESP_LOGI(TAG,"十六进制编码 ciphertext:%s",ciphertext);
...
int priv_rsa_sign(unsigned char* plaintext, unsigned int plaintext_len, unsigned char* sign_output)
{
    ...
    rsa = (mbedtls_rsa_context*)heap_caps_malloc(sizeof(mbedtls_rsa_context), MALLOC_CAP_SPIRAM);
    if(rsa == NULL)
    {
...
ESP_LOGI(TAG,"sizeof(mbedtls_rsa_context) =%d \n",sizeof(mbedtls_rsa_context));
   ...
   uint8_t sig_buff[sig_len];
    ...
    // /***** base64编码 *****/
    // b64_out = malloc(sig_len*2);//必须要*2！！！
    // ret = mbedtls_base64_encode((unsigned char *)b64_out, sig_len*2,
    ...
    // strncpy((char *)ciphertext, (bin_to_hex(sig_buff, (size_t)sig_len)), sig_len);
    /***** 十六进制编码 *****/
    ESP_LOGI(TAG,"十六进制编码 sign_output:%s",sign_output);
...
{
    rsa_public_key.key = NULL;
    rsa_public_key.len = 0;
...
{  
    char *hex_str = (char *)heap_caps_malloc((bin_len * 2) + 1, MALLOC_CAP_SPIRAM); // 每个字节需要两个字符，加上一个字符串结束符  
