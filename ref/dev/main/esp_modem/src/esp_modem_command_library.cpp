...
/*
NL668:"AT+CCID?\r"
EC25:"AT+QCCID\r""AT+QCCID=?\r"

*/
command_result get_iccid(CommandableIf *t, std::string &out)
{
    ESP_LOGV(TAG, "%s", __func__ );
    return generic_get_string(t, "AT+CCID?\r", out, 5000);//"AT+CCID\r"  NL668

//    return generic_get_string(t, "AT+QCCID\r", out, 5000);//"AT+CCID\r"  EC25
...
#define AT_WAIT_TIME2            5000                 // 等待应答超时时间
#define AT_WAIT_TIME_RESET 15000//15s
...
NL668-广和通 
*/
command_result Cellular_Init_PPP(CommandableIf *t)
{
//	uint32_t timeout_ms=0;
	static uint32_t Cellular4G_AT_fail_cnt=0;
	
	uint8_t i=0;
	std::string out;
	int state2=0;
	int &state =state2;
	command_result ret;//= generic_get_string(t, "AT+CSQ", out);//auto
//"\r\n"
//	ESP_LOGV(TAG, "%s", __func__ );
	ESP_LOGE(TAG, "Cellular_Init_PPP");
	Cellular4G_AT_fail_cnt++;
...
//		reset_4g_state();
		 ...
		 //4G模块异常响应超时，发重启命令
		 if(Cellular4G_AT_fail_cnt >= 180)//~180s
		 {
			 Cellular4G_AT_fail_cnt =0;
			  // 关闭GPS
			 if(command_result::OK != generic_command(t,"AT+GTGPSPOWER=0\r", "OK","ERROR",	AT_WAIT_TIME2)) { 
	 // 		 return command_result::FAIL;
			 }
			 // 重启模块
			 if(command_result::OK != generic_command(t,"AT+RESET\r", "AT command ready","ERROR",  AT_WAIT_TIME_RESET)) { 
				 return command_result::FAIL;
			 }
			 return command_result::OK;
		...
		if(command_result::OK != generic_command(t,"AT\r", "OK","ERROR",  AT_WAIT_TIME2)) { 
			return command_result::FAIL;
		}
	
		// 取消回显 			
		if(command_result::OK != generic_command(t,"ATE0\r", "OK",  "ERROR",  AT_WAIT_TIME2)){
			return command_result::FAIL;
		}
		
		// 读取模块SN码 每个模块唯一,imei
		if(command_result::OK != generic_command(t,"AT+CGSN\r","OK", "ERROR",  AT_WAIT_TIME2)){
			return command_result::FAIL;
		}
	
		// 读取模块固件版本号
		if(command_result::OK != generic_command(t,"AT+CGMR\r", "OK",  "ERROR",  AT_WAIT_TIME2)){
			return command_result::FAIL;
		}
		// 查询核心板能否读到SIM卡(必选)
		if(command_result::OK != generic_command(t,"AT+CPIN?\r", "+CPIN: READY", "ERROR",  AT_WAIT_TIME2))
		{
			return command_result::FAIL;
		}
		else
		...
		// 查询移动设备的国际移动用户识别码
		if(command_result::OK != generic_command(t,"AT+CIMI\r", "OK",  "ERROR",  AT_WAIT_TIME2)){ 			   
			return command_result::FAIL;
		}
	
		// 查询模块固件版本号，便于问题分析
		if(command_result::OK != generic_command(t,"ATI\r", "OK",  "ERROR",  AT_WAIT_TIME2)){ 			   
			return command_result::FAIL;
		}
		// 设置正常工作模式
		if(command_result::OK != generic_command(t,"AT+CFUN?\r", "+CFUN: 1",  "ERROR",  AT_WAIT_TIME2)){				  
			if(command_result::OK != generic_command(t,"AT+CFUN=1\r", "OK", "ERROR",  AT_WAIT_TIME2)){
				return command_result::FAIL;
			}
		}
...
//		iot_4g_state.bit.sim_missing = 0;
		...
		for(i = 0; i < TRY_CNT; i++)//40
		{
			ret = generic_get_string(t, "AT+CSQ\r", out,  AT_WAIT_TIME2);
			if (ret != command_result::OK) {
				
			ESP_LOGE(TAG, "Cellular_Init_PPP:1");
				return ret;
			}
			...
			ESP_LOGE(TAG, "Cellular_Init_PPP:2");
				return command_result::FAIL;
			}
			
			if (std::from_chars(out.data() + pos, out.data() + out.size(), state).ec == std::errc::invalid_argument) {
				ESP_LOGE(TAG, "Cellular_Init_PPP:3");

				return command_result::FAIL;
			}			
			if(state2 >= 10 && state2 <= 31)
			{ 
				ESP_LOGE(TAG, "Cellular_Init_PPP:4");
			
//				iot_4g_state.rssi = state;
				break; // 信号强度在此区间才进行下一步
			}			
			if(i == (TRY_CNT-1))
			{
				ESP_LOGE(TAG, "Cellular_Init_PPP:5");
			
				return command_result::FAIL;
			}
			ESP_LOGE(TAG, "Cellular_Init_PPP:6");
			
			vTaskDelay(pdMS_TO_TICKS(10)); // 间隔0.01s查询一次信号强度
		}
		...
		for(i = 0; i < TRY_CNT; i++)//40
		{
			ret = generic_get_string(t, "AT+CREG?\r", out,  AT_WAIT_TIME2);
			if (ret != command_result::OK) {
				return ret;
			}
			...
			if (std::from_chars(out.data() + ber_pos + 1, out.data() + out.size(), state).ec == std::errc::invalid_argument) {
				return command_result::FAIL;
			}
			...
			{
				ESP_LOGE(TAG, "Cellular_Init_PPP:12");
...
//				iot_4g_state.bit.sim_inactive = 1;
		...
		// 设置附着
		if(command_result::OK != generic_command(t,"AT+CGATT=1\r","OK","ERROR",  AT_WAIT_TIME2))
		{
			return command_result::FAIL;
		}
		...
		for(i = 0; i < TRY_CNT; i++)//40
		{
			if(command_result::OK != generic_command(t,"AT+CGATT?\r", "+CGATT: 1", "ERROR",  AT_WAIT_TIME2)){				  
				break;
			}
			if(command_result::OK != generic_command(t,"AT+CGATT=1\r","OK","ERROR",  AT_WAIT_TIME2)){
				return command_result::FAIL;
		...
		if(command_result::OK != generic_command(t,"AT+COPS?\r","+COPS: 0","ERROR",  AT_WAIT_TIME2)){ // 查询网络注册状态
//			ESP_LOGE(TAG, "check cops failed");
			if(command_result::OK != generic_command(t,"AT+COPS=0\r","OK","ERROR",  AT_WAIT_TIME2)){
				return command_result::FAIL;
			}
		}
//		if(command_result::OK != generic_command(t,"AT+GTGPSPOWER=1\r", "OK", "ERROR",  AT_WAIT_TIME2)){
//			return command_result::FAIL;
//		}
...
//		}
		...
		//  
		if(command_result::OK != generic_command(t,"AT+CGREG?\r","OK","ERROR",  AT_WAIT_TIME2))//CGREG:0,1
		{
			return command_result::FAIL;
		}
		//
//		if(command_result::OK != generic_command(t,"AT+CGDCONT=1,\"IP\",\"CMNET\"\r","OK","ERROR",  AT_WAIT_TIME2))//OK
//		{
//			return command_result::FAIL;
//		}
