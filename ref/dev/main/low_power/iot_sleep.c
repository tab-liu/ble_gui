...
static uint8_t iot_sleep_start(void)
{
    ...
    /*停止蓝牙广播*/
    if(1 == SetData.dev_info_t.on_off.bit.ble_enable) iot_ble_stop();
...
#ifdef CONFIG_USB_4G_MODULE_ENABLE
    /*关闭4G任务*/
    if(1 == SetData.dev_info_t.on_off.bit._4g_enable) iot_4G_stop();
#endif

#ifdef CONFIG_WIFI_AP_MODE_ENABLE    
    /*关闭WIFI  AP模式*/
    if(1 == SetData.dev_info_t.on_off.bit.wifi_ap_enable) iot_wifi_ap_deinit();    
#endif 
...
static uint8_t iot_sleep_end(void)
{
...
#ifdef CONFIG_WIFI_AP_MODE_ENABLE    
    /*恢复WIFI  AP模式*/
    if(1 == SetData.dev_info_t.on_off.bit.wifi_ap_enable) iot_wifi_ap_reinit();    
#endif
    ...
    /*恢复蓝牙广播*/
    if(1 == SetData.dev_info_t.on_off.bit.ble_enable) iot_ble_start();
#endif
...
void Sleep_Task_Create(void)
{
    if ( !iot_sleep_task_handle && (1 == reals.IOT_Status_Flag.sBit.system_sleep_flag)) {
        if (xTaskCreate(iot_sleep_task, "iot_sleep_task", 3584, NULL, 1, &iot_sleep_task_handle) == pdPASS) {
...
static uint8_t iot_sleep_twai_stop(void)
{
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_ERR_INVALID_STATE) {
            ESP_LOGW(TAG, "TWAI driver not in running state");
        } else {
        ...
        if (ret == ESP_OK) {
            // ESP_LOGW(TAG, "TWAI RX queue cleared successfully");
        } else {
            ESP_LOGW(TAG, "Failed to clear TWAI RX queue: %s", esp_err_to_name(ret));
        }
    }
    ...
    ESP_LOGW(TAG, "TWAI driver stopped and uninstalled successfully");
    return 1;
}
...
static uint8_t iot_sleep_twai_restart(void)
{
    ...
    ESP_LOGW(TAG, "Restarting TWAI driver");
    esp_canbus_init();

    // /*由于休眠时只停止了驱动而未卸载，这里只需重新启动*/
    // ret = twai_start();
    // if (ret != ESP_OK) {
    //     ESP_LOGE(TAG, "Failed to restart TWAI driver: %s", esp_err_to_name(ret));
    //     /*如果启动失败，尝试重新初始化整个驱动*/
    //     ESP_LOGW(TAG, "Attempting to reinitialize TWAI driver completely");
    //     esp_canbus_init();
    // } else {
    //     ESP_LOGW(TAG, "TWAI driver restarted successfully");
    // }
