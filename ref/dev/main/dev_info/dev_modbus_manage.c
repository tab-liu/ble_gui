...
sMdAddrInfo_t gDevModbusAddrList[DEV_MODBUS_ADDR_NUM_MAX];
uint16_t gDevModbusGroupAddrList[DEV_MODBUS_GROUP_ADDR_NUM_MAX];
...
void dev_modbus_addr_manage_init(void)
{
	...
	dev_modbus_dev_add(self_type, iot_factory.iot_sn, 0, 0, 0, 0);
}
...
//	xSemaphoreGive(xDevManageSemaphore);  /* 释放信号量 */
