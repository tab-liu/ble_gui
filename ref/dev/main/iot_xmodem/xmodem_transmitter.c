...
#define TAG 	"[XMD_SENDER]"
xmodem_server_struct gXmodem_server_Status;//xmodem OTA 主机侧变量
...
static const uint32_t TRANSFER_C_TIMEOUT = 10000; 				// 6 seconds ---> 10s
static const uint32_t TRANSFER_ACK_TIMEOUT = 10000; 				// 6 seconds
static const uint32_t TRANSFER_EOT_TIMEOUT = 10000; 				// 6 seconds
...
static const uint8_t WRITE_BLOCK_MAX_RETRIES = 5; 				// max 5 retries per block
static const uint8_t EOT_MAX_RETRIES = 5;                       // max 5 retries per EOT
...
uint8_t xmodem_transmit_otapct_get(void)
{
    ...
    if (0 == total_packet)
    {
        ESP_LOGE(TAG, "xmodem_transmit_otapct_get total_packet is 0");
        return 0;
    }

    return (uint8_t)(send_packet_idx*100/total_packet);
}
...
uint8_t xmodem_transmit_init(uint32_t size)
{
		...
		(0 == size % XMODEM_BLOCK_SIZE))
	{
		...
		{
			current_packet = iot_malloc(sizeof(xmodem_packet_t));
			if (current_packet == NULL)
			{
		...
		{
			internal_buffer = iot_malloc(XMODEM_BLOCK_SIZE);
			if (internal_buffer == NULL)
			{
...
uint8_t Xmodem_Server_top(channel_type channel,const uint32_t current_time)
{
	static uint32_t stopwatch = 0;
	static uint32_t stopwatch_eot = 0;
	static uint32_t stopwatch_etb = 0;
    static uint8_t client_cnt = 0;      //正确接收到应答响应的客户端设备计数器
	...
	switch (transmit_state)
	{
		...
		{
		#if 0
			if (current_time > (write_block_timer + TRANSFER_C_TIMEOUT))    //lxy：强制等待TRANSFER_C_TIMEOUT后再进行判断
			{
//				if(is_inbound_empty())
				{
...
//                xmd_state_update();
				...
				xmd_get_buffer(payload_buffer_position, internal_buffer, XMODEM_BLOCK_SIZE);
				...
				xmd_write_data(XMODEM_BLOCK_SIZE, internal_buffer);
		...
		{
			if (current_time > (stopwatch + TRANSFER_ACK_TIMEOUT))      //lxy：强制等待TRANSFER_ACK_TIMEOUT后再进行判断
			{
//				if(is_inbound_empty())
				{
					transmit_state = XMODEM_TRANSMIT_WRITE_BLOCK_FAILED;
...
//				else
		...
		case XMODEM_TRANSMIT_WRITE_BLOCK_FAILED:				// 写数据块失败
		{
			if (WRITE_BLOCK_MAX_RETRIES <= write_block_retries)
			{
                ...
                {
                    // 更新待升级设备的数量，并开始传输下一个数据段
                    uart_ota_state.online_client_cnt = client_cnt;
                    transmit_state = XMODEM_TRANSMIT_C_ACK_RECEIVED;
                    ESP_LOGW(TAG, "server state: have %d client received ack", uart_ota_state.online_client_cnt);
                }
			}
			else
			{
...
//		    xmd_state_update();
		...
		{
            client_cnt = 0;             //重置每个数据段的正确应答设备计数器
            write_block_retries = 0;    //重置数据段的重试次数计数器
...
//		    xmd_state_update();
		...
		{
			if (current_time > (stopwatch_eot + TRANSFER_EOT_TIMEOUT))   //lxy：强制等待TRANSFER_EOT_TIMEOUT后再进行判断
			{
//				if(is_inbound_empty())
				{
...
//				else
		...
		{
            if (EOT_MAX_RETRIES <= eot_retries)
            {
			    transmit_state = XMODEM_TRANSMIT_ABORT_TRANSFER;
            ...
            else
            {
                transmit_state = XMODEM_TRANSMIT_WRITE_EOT;
                ++eot_retries;
                ESP_LOGW(TAG, "server state: retry EOT, due to EOT timeout");
            }
			break;
		}
...
//		    xmd_state_update();
		...
		{
			if (current_time > (stopwatch_etb + TRANSFER_ETB_TIMEOUT))  //lxy：强制等待TRANSFER_ETB_TIMEOUT后再进行判断
			{
//				if(is_inbound_empty())
				{
