...
#define RESPONSE_TIMEOUT_USEC       0  // 0.5 s
#define RECEIVE_TIMEOUT_SEC         0
#define RECEIVE_TIMEOUT_USEC        0  // 0.5 s
...
EXT_RAM_BSS_ATTR static modbus_tcp_server_t md_tcp_svr;
EXT_RAM_BSS_ATTR static modbus_addr_map_t g_modbus_addr[REGISTER_NUM_MAX];
EXT_RAM_BSS_ATTR static modbus_addr_map_t segments[REGISTER_NUM_MAX];
...
void modbus_tcp_server_init(void)
{
    ...
    md_tcp_svr.query = heap_caps_malloc(MODBUS_TCP_MAX_ADU_LENGTH, MALLOC_CAP_SPIRAM);
    // md_tcp_svr.query = (uint8_t *)calloc(MODBUS_TCP_MAX_ADU_LENGTH, 1);
...
int _modbus_receive_msg(uint8_t *msg)
{
    ...
    /* 判断是否可读，避免阻塞;
    注意：tcps使用的是判断socket是否有数据，在解密时已经把数据读取出来了，故不能每次读取一部分后判断socket是否有数据。 */
    ...
    if (rc == 0)
    {
        // ESP_LOGI(TAG, "modbus tcp slave timeout, msg_length:%d", msg_length);
        return 0;
    }
    ...
    while (length_to_read != 0)
    {
        ...
        if (md_tcp_svr.config.block && length_to_read > 0 &&  INDICATION_TIMEOUT_SEC == 0 && INDICATION_TIMEOUT_USEC == 0)
        {
            p_tv = NULL;
        }
        else
...
int modbus_addr_pub_to_pri(unsigned char* data)
{
    ...
    uint16_t number = ((data[4] << 8) & 0xFF00) | data[5];
    uint8_t func = data[1];
...
static int modbus_data_handle_read_multi_regs(int sockfd, uint8_t *data, modbus_addr_map_t *segments, int segment_count)
{
    ...
    for (int i = 0; i < segment_count; i++)
    {
        ...
        int16_t rsp_len = Modbus_Slave(&temp_req[_MODBUS_TCP_DATA_OFFSET], temp_req_len + 2,
                                    ...
                                    dummy_can_cmd.cmd, &dummy_can_cmd.num, MD_CHL_BLE, &position_list);
#elif CONFIG_UART_SUPPORT
        reg_position_list_t *position_list = NULL;
        int16_t rsp_len = Modbus_Slave(&temp_req[_MODBUS_TCP_DATA_OFFSET], temp_req_len + 2,
                                    &temp_rsp[_MODBUS_TCP_DATA_OFFSET], MD_CHL_BLE, &position_list);
#else
        int16_t rsp_len = Modbus_Slave(&temp_req[_MODBUS_TCP_DATA_OFFSET], (uint16_t)temp_req_len + 2,
                            &temp_rsp[_MODBUS_TCP_DATA_OFFSET]);
#endif
...
static int modbus_data_handle_write_multi_regs(int sockfd, uint8_t *data, modbus_addr_map_t *segments, int segment_count, int reg_count)
{
    ...
    for (int i = 0; i < segment_count; i++)
    {
        ...
        int16_t rsp_len = Modbus_Slave(&temp_req[_MODBUS_TCP_DATA_OFFSET], temp_req_len + 2,
                                &temp_rsp[_MODBUS_TCP_DATA_OFFSET],
                                can_cmd.cmd, &can_cmd.num, MD_CHL_BLE, &position_list);
#elif CONFIG_UART_SUPPORT
        reg_position_list_t *position_list = NULL;
...
static int modbus_data_handle(int sockfd, uint8_t *data, int len)
{
    ...
    rsp_len = Modbus_Slave(&data[_MODBUS_TCP_DATA_OFFSET], (uint16_t)(len - _MODBUS_TCP_DATA_OFFSET + 2),
                    &ble_resp_data[_MODBUS_TCP_DATA_OFFSET],  can_cmd.cmd, &can_cmd.num, MD_CHL_BLE, &position_list); /* modbus handle */
#else
    reg_position_list_t *position_list = NULL;
    rsp_len = Modbus_Slave(&data[_MODBUS_TCP_DATA_OFFSET], (uint16_t)(len - _MODBUS_TCP_DATA_OFFSET + 2),
                    &ble_resp_data[_MODBUS_TCP_DATA_OFFSET], MD_CHL_BLE, &position_list); /* modbus handle */
#endif
    if (rsp_len > 0)
    {
        len = 0;
...
{
    if (SetData.dev_info_t.modbus_tcp_port == 0)
    {
    ...
    snprintf(service, size, "%d", SetData.dev_info_t.modbus_tcp_port);
