...
extern uint8_t Modbus_21000_semaphore_Take(void);
extern void Modbus_21000_semaphore_Give(void);
#endif
...
// MQTT上报接口
extern void Mqtt_Active_Report_Queue_Push(uint8_t *pdata, uint16_t len, uint8_t scene);
...
static void modbus_tlv_send(const uint8_t *data, uint16_t len, uint8_t chl)
{
    ...
    switch (chl) {
        ...
        case MD_CHL_WIFI_CLOUD:
            Mqtt_Active_Report_Queue_Push((uint8_t *)data, len, 0x0F);
            break;
...
void modbus_tlv_queue_init(size_t capacity)
{
    ...
    if (xQueue_modbus_tlv_rx == NULL) {
        ESP_LOGE(TAG, "modbus_tlv_queue_init: xQueueCreate failed (cap=%u)", (unsigned)capacity);
    } else {
        ESP_LOGI(TAG, "modbus_tlv_queue_init: created (cap=%u)", (unsigned)capacity);
    }
}
 ...
 * @param chl 发送通道
 */
static void modbus_tlv_read_rtn_send_frame(uint16_t payload_len, uint8_t total_index, uint8_t curr_index, uint8_t chl)
{
    uint16_t crc16 = 0;

    modbus_tlv_tx_buffer[0]  = MD_SUMMARY_ADDR;
    modbus_tlv_tx_buffer[1]  = 0x10;
    modbus_tlv_tx_buffer[2]  = MOD_REG_START_ADDR_21000 >> 8;
    modbus_tlv_tx_buffer[3]  = MOD_REG_START_ADDR_21000 & 0xFF;
    modbus_tlv_tx_buffer[4]  = ((payload_len + 4) / 2) >> 8;
    modbus_tlv_tx_buffer[5]  = ((payload_len + 4) / 2) & 0xFF;
    modbus_tlv_tx_buffer[6]  = payload_len + 4;
    modbus_tlv_tx_buffer[7]  = (LSB2MSB(MODBUS_21000_FUNC_TLV_DATA)) & 0xFF;
    modbus_tlv_tx_buffer[8]  = (LSB2MSB(MODBUS_21000_FUNC_TLV_DATA) >> 8) & 0xFF;
    modbus_tlv_tx_buffer[9]  = total_index;
    modbus_tlv_tx_buffer[10] = curr_index;

#if 1   // 按MModbus协议，转换为大端格式
    /* 对 TLV 数据区按 16 位进行高低字节转换 */
    for (uint16_t offset = 11; (offset + 1) < (uint16_t)(11 + payload_len); offset += 2) {
        uint16_t word = 0;
        memcpy(&word, &modbus_tlv_tx_buffer[offset], sizeof(word));
        word = U16_TO_BE(word);
        memcpy(&modbus_tlv_tx_buffer[offset], &word, sizeof(word));
    }
#endif

    crc16 = ModbusCrc16(modbus_tlv_tx_buffer, payload_len + 11);
    modbus_tlv_tx_buffer[payload_len + 11] = crc16 & 0xFF;
    modbus_tlv_tx_buffer[payload_len + 12] = (crc16 >> 8) & 0xFF;

    modbus_tlv_send(modbus_tlv_tx_buffer, payload_len + 13, chl);
}
...
static int modbus_tlv_process_read_unit(uint16_t **p_tab, const modbus_tlv_unit_t *tl_data, uint8_t chl)
{
    ...
    *p_tab = NULL;
    ...
    reg_position_t reg_position = {
        .reg_addr = tl_data->reg_addr,
        .offset = 0,
        .len = reg_nums,
    };
    /* 查询目标寄存器表地址 */
    *p_tab = vLookupDataTab(tl_data->slave_addr, tl_data->reg_addr, reg_nums, false, &reg_position);
    if (*p_tab == NULL) {
 ...
 */
static uint8_t modbus_tlv_read_rtn_calc_total_index(uint8_t tab_total)
{
    ...
    for (uint8_t curr_tab = 0; curr_tab < tab_total; ++curr_tab) {
        ...
        if ( modbus_tlv_read_unit[curr_tab].value ) {
            curr_tlv_len = modbus_tlv_read_unit[curr_tab].len;
        } else {
            curr_tlv_len = 0;
        }

        /* 放 TL 头之前，先确认本帧还能否容纳一个完整 TL 头 */
        if (payload_len > (MODBUS_TLV_DATA_LEN_MAX - MODBUS_TL_LEN)) {
            total_index++;
            payload_len = 0;
        }
        ...
        while (curr_tlv_len > 0) {
            uint16_t free_len = MODBUS_TLV_DATA_LEN_MAX - payload_len;
...
static void modbus_tlv_read_rtn_framing_and_send(uint8_t tab_total, uint8_t chl)
{
    ...
    total_index = modbus_tlv_read_rtn_calc_total_index(tab_total);
    ...
    while (curr_tab < tab_total) {
        ...
        if (tlv_send_len > (MODBUS_TLV_DATA_LEN_MAX - MODBUS_TL_LEN)) {
            modbus_tlv_read_rtn_send_frame(tlv_send_len, total_index, curr_index++, chl);
            tlv_send_len = 0;
        }
...
static void modbus_tlv_read_cmd_process(uint8_t *data, uint16_t data_len, uint8_t chl)
{
    ...
    while (remain_len >= MODBUS_TL_LEN) {
        ...
        /* 处理TL单元，寻找对应Value位置 */
        if (modbus_tlv_process_read_unit(&p_tab, tl_data, chl) == 0) {
...
static int modbus_tlv_process_write_unit(const modbus_tlv_unit_t *tl_data, uint8_t chl)
{
    ...
    p_tab = vLookupDataTab(tl_data->slave_addr, tl_data->reg_addr, reg_nums, true, &reg_position);
    if (p_tab == NULL) {
