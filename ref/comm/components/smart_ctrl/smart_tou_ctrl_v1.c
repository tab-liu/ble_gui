...
static bool Smart_Tou_Is_Ctrl_Valid(tou_time_quantum_t *old_quantum)
{
    ...
    /* 全部字段均匹配则认为控制有效 */
    return (is_type_present & is_dev_id_present & is_addr_present);
}
    ...
    for (uint8_t old_index = 0; old_index < SMART_TOU_QUANTUM_MAX_CNT; ++old_index) {
        ...
        if (is_start_present && is_end_present) {
            ...
            if (Smart_Tou_Is_Ctrl_Valid(old_quantum)) {
                bool is_start_value = (old_quantum->ctrl.start_value == Smart_Tou_Ctrl_Config.smart_tou_unit[start_idx].reg_cfg.reg_value) ? true : false;   
                bool is_end_value = (old_quantum->ctrl.end_value == Smart_Tou_Ctrl_Config.smart_tou_unit[end_idx].reg_cfg.reg_value) ? true : false;   
                if ( is_start_value && is_end_value) {
                    ...
                    for (uint8_t gq = 0; gq < SMART_TOU_QUANTUM_MAX_CNT; ++gq) {
                        if (Time_Quantum_Config[gq].start_time_index == start_idx &&
                            Time_Quantum_Config[gq].end_time_index   == end_idx) {
                            // 时间段仅变动了起止点位控制值，对于正在生效的时间段，不执行旧的结束点位，直接执行新的起始点位
                            if ( TOU_PERIOD_STATUS_ACTIVE == Time_Quantum_Config[gq].status ) {
                                Time_Quantum_Config[gq].status = TOU_PERIOD_STATUS_CONFIGURED_IDLE;
                            }
                            break;
                        }
                    }
        ...
        if (day_first_running_flag) {
            ...
            for (uint8_t q = 0; q < SMART_TOU_QUANTUM_MAX_CNT; ++q) {
                if (Time_Quantum_Config[q].status == TOU_PERIOD_STATUS_ACTIVE) {
                    uint8_t start_idx = Time_Quantum_Config[q].start_time_index;
        ...
        switch (tq->status) {
            case TOU_PERIOD_STATUS_CONFIGURED_IDLE:
                ...
                if (time_sec >= start_sec && time_sec < end_sec) {
                    Smart_Tou_Enqueue_Result_ByIndex((int)start_idx); /* enqueue start op */
