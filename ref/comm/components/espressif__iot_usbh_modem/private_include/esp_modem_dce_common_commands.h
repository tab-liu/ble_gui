 ...
 * @param[in]  dce     Modem DCE object
 * @param[in]  param   size_t of output string length (casted to void*), max size the resultant string
 * @param[out] result  Pointer to the string where the resultant ICCID number gets copied (if size param fits)
 *
 * @return
 *      - ESP_OK on success
 *      - ESP_FAIL on error
...
/**
 * @brief Send a custom AT command to the modem DCE and get the response.
 *
 * @param[in]  dce      Modem DCE object
 * @param[in]  command  AT command string to send
 * @param[in]  param    Additional parameter (e.g., expected response length or timeout)
 * @param[out] result   Pointer to buffer for storing the response string
 *
 * @return esp_err_t
 *      - ESP_OK on success
 *      - ESP_FAIL on error
