...
{
    ESP_LOGD("esp_modem_dce_common_handle", "%s", line);
    ...
    if (strstr(line, "+")) {
        if (result_str && result_str->command != NULL && result_str->string != NULL && result_str->len != 0) {
        ...
        if (result_str && result_str->command != NULL && result_str->string != NULL && result_str->len != 0) {
            char *result = line;
            char *result_target = NULL;
            while (*result == ' ' || *result == '\t' || *result == '\r' || *result == '\n') result++;
            size_t last_result_len = strnlen(result_str->string, (result_str->len - 1));
            if ((0 < last_result_len) && ( 0 == memcmp(result, MODEM_RESULT_CODE_SUCCESS, 2))) {
                if(result_str->string[last_result_len] == '\0') result_str->string[last_result_len] = '\n';
                last_result_len++;
                result_target = result_str->string + last_result_len;
            } else {
                result_target = result_str->string;
            }
            int len = snprintf(result_target, (result_str->len - last_result_len), "%s", result);
            if (len > 2) {
                /* Strip "\r\n" */
                strip_cr_lf_tail(result_str->string, (len + last_result_len));
            }
            err = ESP_OK;
        }
    }
...
{
    common_string_t common_str = { .command = "AT+CGSN=1\r", .string = result, .len = (size_t)param };
...
{
    common_string_t common_str = { .command = "AT+CCID?\r", .string = result, .len = (size_t)param };
    return common_get_common_string(dce, &common_str);
}
...
esp_err_t esp_modem_dce_common_send_at_cmd(esp_modem_dce_t *dce, const char *command, void *param, void *result)
{
    common_string_t common_str = { .command = command, .string = result, .len = (size_t)param };
    return common_get_common_string(dce, &common_str);
}
