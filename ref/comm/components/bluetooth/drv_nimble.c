...
#else
static uint8_t own_addr_type = 0;                           // 蓝牙地址类型
#endif
static uint16_t ble_conn_handle = BLE_HS_CONN_HANDLE_NONE;  // 蓝牙连接句柄
static uint8_t notify_flag1 = 0;                            // 特征1通知标志
static uint8_t notify_flag3 = 0;                            // 特征3通知标志
static app_rx_callback_t app_rx_cb = NULL;                  // 应用层接收数据回调函数
static nimble_event_cb_t nimble_event_cb = NULL;            // ble事件回调函数
...
int16_t set_ble_adv_data(uint8_t *ad_data, uint8_t ad_len)
{
    //判断蓝牙驱动是否已经初始化
    if (!drv_inited) {
        ESP_LOGE(TAG, "ble driver not initialized");
        return -1;
    }
...
//    BLE_HS_ADV_TYPE_MFG_DATA=0xFF
    ...
    // 只有在API调用成功后，才将逻辑状态设置为 true
    ble_adv_logically_on = true;

    ESP_LOGW(TAG, "ble advertise successfully, name: %s, conn_mode: %d", ble_info.dev_name, conn_mode);
    if (nimble_event_cb) {
        nimble_event_cb(NIMBLE_EVT_ADV, NULL);
    }
    return 0;
}
#endif
...
static int nimble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) 
        {
            ...
            if (nimble_event_cb) {
                nimble_event_cb(NIMBLE_EVT_CONNECT, NULL);
            }
			
            /* 设置期望的连接参数，最终由central设备决定 */
            struct ble_gap_upd_params conn_params = {0};
            conn_params.itvl_min = BLE_GAP_CONN_ITVL_MS(15);    // 15ms (15/1.25)
            conn_params.itvl_max = BLE_GAP_CONN_ITVL_MS(15);    // 15ms (15/1.25)
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        if (nimble_event_cb)
        {
            ...
            nimble_event_cb(NIMBLE_EVT_DISCONNECT, &reason);
        }
		
        drv_ble_advertise_pause();
        if ( 1 == iot_ble_on_off_status() )
        {
            ESP_LOGW(TAG, "ble disconnect, reason=%d, and ble resume advertising", event->disconnect.reason);
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        if ( event->conn_update.status ) {
            // event->conn_update.status != 0 表示 central 拒绝或协商失败（update 未被应用
            ESP_LOGE(__func__, "central update connection parameters, status=%d", event->conn_update.status);
        } else {
            ESP_LOGI(__func__, "conn update success");
        }
        rc = ble_gap_conn_find(event->conn_update.conn_handle, &desc);
        if (rc == 0) {
            ESP_LOGI(__func__,
                "conn_interval=%d(%gms) conn_latency=%d(%dms) supervision_timeout=%d(%dms)",
                desc.conn_itvl, desc.conn_itvl * 1.25f,
                desc.conn_latency, desc.conn_latency * (int)(desc.conn_itvl * 1.25f), // 可选
            ...
            ESP_LOGE(__func__, "ble_gap_conn_find failed rc=%d, desc not valid", rc);
            ble_info.rx_timeout = 500;
        }

        ESP_LOGI(__func__, "receive ble message timeout = %ums", ble_info.rx_timeout);
        return 0;

        /* 蓝牙广播完成，蓝牙可设置每次广播的时间，当广播时间达到后则广播完成
         * 广播完成后继续开启广播 */
    case BLE_GAP_EVENT_ADV_COMPLETE:
        ESP_LOGW(__func__, "advertise complete, reason=%d, ble_adv_logically_on=%d", event->adv_complete.reason, (uint8_t)ble_adv_logically_on);
        if(ble_adv_logically_on == false)  return 0;
#if BLE_CONFIG_EXTENDED_ADV
        ext_drv_ble_advertise();
#else
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        if (gatts_get_chr1_handle() == event->subscribe.attr_handle) {
            ...
            if (nimble_event_cb)
            {
                nimble_event_cb(NIMBLE_EVT_SUBSCRIBE, &notify_flag1);
            }
        }
    ...
    case BLE_GAP_EVENT_MTU:
        ...
        ESP_LOGI(__func__, "mtu update event, conn_handle=%d cid=%d mtu=%d(result=%d)",
...
static void nimble_on_sync(void)
{
    ...
    ext_drv_ble_advertise();
#else
	drv_ble_advertise(BLE_GAP_CONN_MODE_UND);    // 蓝牙广播从此处开启,as server
#endif
}
...
#ifdef BLE_CLIENT_ENABLE
	 ESP_LOGE(TAG, "ble client peer_init " );

	 /* Initialize data structures to track connected peers. */
	 ret = peer_init(MYNEWT_VAL(BLE_MAX_CONNECTIONS), 10, 10, 10);//(MYNEWT_VAL(BLE_MAX_CONNECTIONS), 10, 10, 10)(MYNEWT_VAL(BLE_MAX_CONNECTIONS), 64, 64, 64)
//	 assert(rc == 0);
	 if (ret != 0)
	 {
		  ESP_LOGE(TAG, "failed to init nimble CC, ret: %d ", ret);
		 return -1;
	 }  
#endif
    ...
    ble_info.mtu_size = BLE_ATT_MTU_DFLT; // 蓝牙MTU默认大小
    ble_info.rx_timeout = 500;
...
int drv_ble_deinit(void)
{
    ...
    drv_inited = false;
    memset(&ble_eventInfo, 0, sizeof(ble_eventInfo));
...
{    
    ...
    // 只有在API调用成功后，才更新逻辑状态
    if(ret == 0) ble_adv_logically_on = false;
...
{
    ESP_LOGI(TAG, "stop advertise");
    drv_ble_disconnect();
    int ret = drv_ble_advertise_pause();
...
{
    ble_info.conn_sta = ble_eventInfo.ble_event.bit.connectEvent;
    return &ble_info;
}
...
uint8_t is_chr3_notify_open(void)
{
    return notify_flag3;
}

/*返回蓝牙初始化状态*/
bool drv_ble_get_init_flag(void)
{
    return drv_inited;
}
