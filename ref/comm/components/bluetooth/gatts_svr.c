...
EXT_RAM_BSS_ATTR static uint8_t gatt_svr_chr2_val[CHR2_BUF_SIZE];    // 特征2属性值变量
...
EXT_RAM_BSS_ATTR static uint8_t gatt_svr_chr3_val[CHR1_BUF_SIZE];    // 特征3属性值变量
static uint16_t gatt_svr_chr3_val_handle;           // 特征3属性句柄

/* 特征FF03描述符定义 */
static char gatt_svr_dsc3_val[] = "nimble: notify data channel";
static gatts_data_rx_callback data_rx_callback;     //FF02 特征2数据接收回调函数,app_data_rx_callback()
            ...
            /* 特征FF01 */
            {
                ...
                { {
                    //   .uuid = &gatt_svr_dsc1_uuid.u,
                      .uuid = BLE_UUID16_DECLARE(BLE_SVC_DSC_CFG_UUID16),
#if GATTS_CONFIG_ENCRYPTION
...
// 流控安全余量：预留部分块给接收、HCI事件等底层操作，避免发送占满全池
#define BLE_FLOW_RSV_MSYS1   8   // MSYS_1 预留空闲块数
#define BLE_FLOW_RSV_MSYS2   2   // MSYS_2 预留空闲块数
...
static int gatts_svr_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR:
		...
		{          // 处理发送给特征2的数据
            ...
            if (chr2_recv_len) {
                ESP_LOG_BUFFER_HEX_LEVEL(TAG, gatt_svr_chr2_val, chr2_recv_len, ESP_LOG_DEBUG);
            }

//////////
#ifdef BLE_CLIENT_ENABLE
	//		ble_resend_up_to_down(ctxt);
...
int gatts_data_tx(uint8_t *data, uint8_t len, uint8_t notify, uint16_t conn_handle)
{
    ...
    do {
        ...
        if (om == NULL) 
        {
            ...
            ESP_LOGE(TAG, "no MBUFs available from pool(msys total: %d, free: %d), retry...(remain: %d)", 
                     os_msys_count(), os_msys_num_free(), retry_cnt);
...
{
    int ret = -1;
    ESP_RETURN_ON_FALSE(notify!=0, -1, "gatts_chr3_data_tx", "notification not opened");
...
{
    // msys1空闲（当前mtu大小决定msys2基本不会被使用）
    int free_msys1 = (os_msys_num_free() > CONFIG_BT_NIMBLE_MSYS_2_BLOCK_COUNT) ? (os_msys_num_free() - CONFIG_BT_NIMBLE_MSYS_2_BLOCK_COUNT) : 0;
    // msys2空闲
    int free_msys2 = CONFIG_BT_NIMBLE_MSYS_2_BLOCK_COUNT;

    // 检查剩余资源是否充足
    if (free_msys1 <= BLE_FLOW_RSV_MSYS1 ||
        free_msys2 <= BLE_FLOW_RSV_MSYS2) {
        return false;
    }
