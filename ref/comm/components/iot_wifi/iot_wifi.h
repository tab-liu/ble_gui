#ifndef IOT_WIFI_INIT_H
#define IOT_WIFI_INIT_H

#include <string.h>
#include <stdint.h>
#include <stdio.h>
#include "esp_wifi.h"
#include "esp_netif.h"
#include "lwip/inet.h"

#define WIFI_MODE_DESC(mode) \
    ((mode) == WIFI_MODE_STA ? "STA" : \
    (mode) == WIFI_MODE_AP ? "AP" : \
    (mode) == WIFI_MODE_APSTA ? "AP+STA" : "NULL")

#define WIFI_Set_Mode_STA   1
#define WIFI_Set_Mode_AP    2
#define WIFI_Set_Mode_ALL   3

#pragma pack(1)

typedef union {
    uint16_t all;
    struct {
        uint8_t enable : 1; // wifi 开启
        uint8_t sta_enable : 2; // 0：关闭，1：打开，2：故障;state, not cmd;STA模式使能情况下，才尝试连接AP
        uint8_t ap_enable  : 2; // 0：关闭，1：打开，2：故障

        uint8_t sta_link_ap : 1; // 0：断开；1：连接
        uint8_t ap_is_link  : 1; // 0：断开；1：被连接

        uint8_t mqtt_login : 1; // 0：未登录，1：登录
        uint8_t smart_cfg  : 1; // 0：关闭，1：开启
    }bit;
}wifi_flags_t;


typedef struct {
    wifi_flags_t  flag;
    uint8_t sta_ipv4[4];
    uint8_t sta_gw[4];
    uint8_t sta_mask[4];
    uint8_t sta_dns1[4];
    uint8_t sta_dns2[4];    
    uint8_t sta_mac[6];
    uint8_t fault_code;
    uint8_t wait_smart_cfg;

    uint8_t sta_index;
    uint8_t sta_auth;
    char sta_ssid[32];
    char sta_pwd[64];
}wifi_info_t;
extern    wifi_info_t wifi_info;//RAM中间变量集合static 

typedef struct {
    uint8_t sta_index;
    uint16_t *sta_auth;
    char *ssid;
    char *password;
}wifi_mmult_sta_base_t;

typedef struct {
    uint8_t sta_index;
    uint32_t *sta_set_ip;              
    uint32_t *sta_set_mask;      
    uint32_t *sta_set_gw;
    uint32_t *sta_set_dns1;
    uint32_t *sta_set_dns2;
}wifi_mmult_sta_static_ip_t;

typedef struct {
    uint8_t chl;
    int rssi;
    bool found;
} sta_scan_result_t;

#pragma pack()

/* 增强的错误分类与字符串化（在文件顶部或合适位置放置 enum/函数） */
typedef enum {
    WIFI_ERR_NONE = 0,
    WIFI_ERR_PASSWORD,     // 密码/加密/握手相关
    WIFI_ERR_AUTH,         // 认证失败
    WIFI_ERR_NO_AP,        // 找不到 AP / SSID 不在范围
    WIFI_ERR_BEACON_LOSS,  // 信标丢失 / 信号不稳
    WIFI_ERR_ASSOC_FAIL,   // 关联失败
    WIFI_ERR_CHANNEL,      // 信道/干扰/切换类
    WIFI_ERR_CAPACITY,     // AP 资源/并发限制
    WIFI_ERR_OTHER
} wifi_err_type_t;

typedef enum {
    INTERNET_OK = 0,
    INTERNET_NO_IP,
    INTERNET_DNS_FAIL,
    INTERNET_NO_ROUTE
} internet_status_t;

enum {
    WIFI_STA_INDEX_UNKNOWN = 0,
    WIFI_STA_INDEX1 = 1,
    WIFI_STA_INDEX2 = 2,
    WIFI_STA_INDEX3 = 3,
    WIFI_STA_INDEX_STOP,
};

#define WIFI_STA_INDEX_MAX 3

const wifi_info_t * iot_get_wifi_info(void);

void iot_wifi_reset(void);
void iot_wifi_close(uint8_t mode);
void iot_wifi_open(uint8_t mode);

void iot_wifi_new_ap(uint8_t auth, const char *ssid, const char *pwd);

void iot_wifi_ap_config(uint8_t auth, const char *ssid, const char *pwd);
int8_t iot_wifi_ap_rssi(void);
void wifi_task_init(void);
void iot_wifi_State_Machine(void);
void iot_wifi_ap_reinit(void);
void iot_wifi_ap_deinit(void);
void iot_wifi_reconnect_in_sleep(bool force);
void iot_wifi_new_connect(uint8_t STA_index);
uint8_t iot_wifi_sta_index_get(void);
void iot_wifi_auto_connect(void);
uint8_t iot_wifi_sta_link_status_get(void);
void iot_wifi_base_cfg_update(void);
void wifi_sta_update_flag_set(bool set);
void iot_wifi_disconnect(void);
void iot_wifi_process_internet_check_result(internet_status_t status);
uint8_t iot_wifi_on_off_status(void);

#endif
