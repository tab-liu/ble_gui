...
typedef struct usbh_cdc_s {
    ...
    const usbh_cdc_user_info_t *user_self_config;
} usbh_cdc_t;
...
static esp_err_t _cdc_reset(usbh_cdc_t *cdc);

static esp_err_t _cdc_check_user(usbh_cdc_t *cdc, const usb_device_desc_t *device_desc);
...
static void usb_event_cb(const usb_host_client_event_msg_t *event_msg, void *arg)
{
    ...
    case USB_HOST_CLIENT_EVENT_NEW_DEV: {
        ...
        SLIST_FOREACH(cdc, &p_usbh_cdc_obj->cdc_devices_list, list_entry) {
            if (_cdc_check_user(cdc, device_desc) != ESP_OK) {
                continue;
            }
...
static esp_err_t _cdc_reset(usbh_cdc_t *cdc)
{
    ...
    cdc->vid = CDC_HOST_ANY_VID;
    cdc->pid = CDC_HOST_ANY_PID;
    if (cdc->user_self_config) {
        if (cdc->intf_idx == cdc->user_self_config->modem_port) {
            cdc->intf_idx = CONFIG_MODEM_USB_ITF;
        } else if (cdc->intf_idx == cdc->user_self_config->second_at_port) {
            cdc->intf_idx = CONFIG_MODEM_USB_ITF2;
        }
        cdc->user_self_config = NULL; // 重置指针
    }
    return ESP_OK;
}
...
{
    if (!cdc || !device_desc) {
        ESP_LOGE(TAG, "Invalid argument: cdc=%p, device_desc=%p", cdc, device_desc);
        return ESP_ERR_INVALID_ARG;
    }

    const usbh_cdc_user_info_t *info = NULL;
    if (find_usbh_cdc_user_info(device_desc->idVendor, device_desc->idProduct, &info) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to find USB CDC user info for VID: 0x%04X, PID: 0x%04X.", device_desc->idVendor, device_desc->idProduct);
        cdc->user_self_config = NULL;
        return ESP_ERR_INVALID_STATE;
    }

    cdc->user_self_config = info;

    if (info->allow_connect) {
        ESP_LOGI(TAG, "Device allowed to connect. VID: 0x%04X, PID: 0x%04X.", device_desc->idVendor, device_desc->idProduct);
        cdc->vid = device_desc->idVendor;
        cdc->pid = device_desc->idProduct;
        if (cdc->intf_idx == CONFIG_MODEM_USB_ITF) {
            cdc->intf_idx = info->modem_port;
            ESP_LOGI(TAG, "Set modem interface index to %d.", info->modem_port);
        } else if (cdc->intf_idx == CONFIG_MODEM_USB_ITF2) {
            cdc->intf_idx = info->second_at_port;
            ESP_LOGI(TAG, "Set second AT interface index to %d.", info->second_at_port);
        } else {
            ESP_LOGW(TAG, "Unknown interface index: %d. No action taken.", cdc->intf_idx);
        }
        return ESP_OK;
    } else {
        ESP_LOGE(TAG, "Device is not allowed to connect. VID: 0x%04X, PID: 0x%04X.", device_desc->idVendor, device_desc->idProduct);
        return ESP_FAIL;
    }
}
...
esp_err_t usbh_cdc_create(const usbh_cdc_device_config_t *config, usbh_cdc_handle_t *cdc_handle)
{
    ...
    if (ret == ESP_OK) {
        if (_cdc_open(cdc) != ESP_OK) {
            // 查找成功但打开失败后继续执行，本次初始化不报错
            ESP_LOGE(TAG, "Failed to open cdc device: %d", cdc->dev_addr);
            _cdc_reset(cdc);
        }
    }
