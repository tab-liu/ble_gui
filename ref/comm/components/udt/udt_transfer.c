...
static udt_transfer_t udt_transfer;					// 传输列表,包含蓝牙端、PC端与服务端的传输函数
...
static uint8_t udt_relay_mode = UDT_MODE_SVC;		// 默认服务器模式
static SemaphoreHandle_t xUdtSemaphore = NULL;      // 申明互斥型信号量
static QueueHandle_t udt_recv_queue = NULL;         // 数据通道接收队列
...
static int udt_protocol_check(uint8_t *data, int size)
{
	...
	{
        ESP_LOGE(TAG, "[udt_protocol_check]  ERROR, mark(%d), version(%d), scene(%d), size(%d)", udt_prot->mark, udt_prot->version, udt_prot->scene, size);
	    return -1;
    }
	...
	if (crc_old != crc_now) {
        ESP_LOGE(TAG, "[udt_protocol_check] crc error, size(%d)", size);
        return -1;
    }
	return 0;
}
...
void udt_init(uint8_t local_addr, udt_transfer_t *transfer, udt_relay_t *relay, udt_evt_callback_t evt_cb)
{
    ...
    /*接收队列初始化*/
	if (udt_recv_queue == NULL) {
		udt_recv_queue = xQueueCreate(16, sizeof(udt_recv_data_t));
		if (udt_recv_queue == NULL) {
			ESP_LOGE(TAG, "udt_recv_queue create failed");
		}
	}
}
...
int udt_relay_to_terminal(uint8_t *data, int size)
{
	/* 参数检测 */
	if ((data == NULL) || (size == 0)) {
        ESP_LOGE(TAG, "[udt_relay_to_terminal] (data == NULL) || (size == 0) error");
        return -1;
    }

	/* 服务端传输检测 */
	if (udt_relay_mode == UDT_MODE_SVC) {
		if (udt_transfer.to_server == NULL) return -1;
	}

	/* PC端传输检测 */
	if (udt_relay_mode == UDT_MODE_PC) {
		if (udt_transfer.to_pc == NULL) return -1;
	}

	/* 蓝牙端传输检测 */
	if (udt_relay_mode == UDT_MODE_BLE) {
...
//        ESP_LOG_BUFFER_HEX_LEVEL(TAG, data, size, ESP_LOG_WARN);
    ...
    ESP_LOGW(TAG, "[udt_relay_to_terminal] funcode(%d), len(%d), id(%d)", udt_prot->funcode, size, UDT_SWAP16(udt_prot->pkg_idx));


	/* 向终端中继数据 */
	if (udt_relay_mode == UDT_MODE_SVC) {
		return udt_transfer.to_server(udt_prot->address, data, size);
    } else if (udt_relay_mode == UDT_MODE_BLE) {
...
{
    uint8_t udt_buffer[UDT_DATA_BUFF_SIZE] = {0};      // 数据缓存
	...
	if ((tx_data->funcode != UDT_FUNCODE_QUERY_FILE_RTN) 
        && (tx_data->funcode != UDT_FUNCODE_QUERY_CONTENT_RTN) 
	...
	/* 服务端传输检测 */
	if (tx_data->mode == UDT_MODE_SVC || tx_data->funcode == UDT_FUNCODE_NEW_LOG_REPORT) {
		if (udt_transfer.to_server == NULL) return -1;
	}

	/* PC端传输检测 */
	if (tx_data->mode == UDT_MODE_PC) {
		if (udt_transfer.to_pc == NULL) return -1;
	}

	/* 蓝牙端传输检测 */
	if (tx_data->mode == UDT_MODE_BLE) {
		if (udt_transfer.to_ble == NULL) return -1;
	}
	...
	udt_prot->version = UDT_PROT_VER;
	udt_prot->scene = UDT_PROT_SCENE;
    if ( tx_data->funcode == UDT_FUNCODE_NEW_LOG_REPORT ) {
        udt_prot->req_id = 0;
    } else {
        udt_prot->req_id = UDT_SWAP16(tx_data->req_id);
    }
	udt_prot->pkg_idx = UDT_SWAP16(tx_data->pkg_idx);
	udt_prot->pkg_total = UDT_SWAP16(tx_data->pkg_total);
    if ( tx_data->funcode == UDT_FUNCODE_QUERY_CONTENT_RTN ) {
        udt_prot->type_read = udt_event.param.qry_cont.type_read;
    } else {
        udt_prot->type_read = 0;
    }
	...
	uint16_t crc_md = crc16_modbus_calc(udt_buffer, len);			// 计算modbus crc
	udt_buffer[len++] = crc_md;
	udt_buffer[len++] = crc_md >> 8;

    ESP_LOGW(TAG, "[udt_transmit] funcode(%d), len(%d), id(%d), mode(%d)", udt_prot->funcode, len, tx_data->pkg_idx, tx_data->mode);
	...
	/* 向终端发送本机数据 */
	if (tx_data->mode == UDT_MODE_SVC || tx_data->funcode == UDT_FUNCODE_NEW_LOG_REPORT) {
		ret = udt_transfer.to_server(udt_prot->address, udt_buffer, len);
    } else if (tx_data->mode == UDT_MODE_BLE) {
        ret = udt_transfer.to_ble(udt_prot->address, udt_buffer, len);
	} else {
		ret = udt_transfer.to_pc(udt_prot->address, udt_buffer, len);
	}
...
int udt_receive(uint8_t *data, int size, uint8_t mode)
{
    ...
    /*清除上次缓存*/
    memset(&udt_event, 0, sizeof(udt_event));
        
	/* 保存事件及其对应参数 */
	udt_event.funcode = udt_prot->funcode;
    udt_event.mode = mode;
	udt_event.req_id = UDT_SWAP16(udt_prot->req_id);
	...
	switch (udt_event.funcode)
	{
		...
		case UDT_FUNCODE_QUERY_CONTENT:					// 终端查询文件内容
			...
			udt_event.param.qry_cont.type_read = udt_prot->type_read;
 ...
 * @return 成功返回0, 否则返回-1
 */
static int udt_pc_interface(uint8_t *data, int size)
{
	/* 参数检测 */
	...
	{
		ret = udt_receive(data, size, UDT_MODE_PC);				// 本机接收
		goto __exit;
	}

	/* 中继到下级设备 */
	ret = udt_relay_to_device(data, size, UDT_MODE_PC);
 ...
 * @return 成功返回0, 否则返回-1
 */
static int udt_svc_interface(uint8_t *data, int size)
{
	/* 参数检测 */
	if ((data == NULL) || (size == 0)) return -1;
	...
	if (udt_prot->address == udt_local_addr) 		// 如果设备地址是本机地址
	{
		ret = udt_receive(data, size, UDT_MODE_SVC);				// 本机接收
		goto __exit;
	}

	/* 中继到下级设备 */
	ret = udt_relay_to_device(data, size, UDT_MODE_SVC);
 ...
 * @return 成功返回0, 否则返回-1
 */
static int udt_ble_interface(uint8_t *data, int size)
{
	/* 参数检测 */
	if ((data == NULL) || (size == 0)) return -1;
	...
	if (udt_prot->address == udt_local_addr) 		// 如果设备地址是本机地址
	{
		ret = udt_receive(data, size, UDT_MODE_BLE);				// 本机接收
		goto __exit;
	}

	/* 中继到下级设备 */
	ret = udt_relay_to_device(data, size, UDT_MODE_BLE);
...
/**
  ...
  * @return     int
  */
static int udt_process_handle(uint8_t *data, int size, uint8_t chl)
{
	/* 参数检测 */
	if ((data == NULL) || (size == 0)) return -1;
...
int udt_recv_push(uint8_t *data, int size, uint8_t chl) {
    ...
    udt_recv_data_t udt_recv_data = {0};

    /*申请内存*/
    udt_recv_data.data = (uint8_t *)iot_calloc(size);
    if ( !udt_recv_data.data ) {
        return -1;
    }

    /*报文同步*/
    memcpy(udt_recv_data.data, data, size);
    udt_recv_data.data_len = size;
    udt_recv_data.chl = chl;

    /*发送到队列*/
    if (udt_recv_queue && xQueueSend(udt_recv_queue, &udt_recv_data, pdMS_TO_TICKS(100)) == pdPASS) {
        return 0;
    } else {
        ESP_LOGE(TAG, "udt_recv_push : xQueueSend failed!");
        iot_free(udt_recv_data.data);
        return -1;
    }
}
...
void udt_recv_process_task(void) {
    udt_recv_data_t udt_recv_data = {0};
    if ( udt_recv_queue && xQueueReceive(udt_recv_queue, &udt_recv_data, 0) == pdPASS ) {
        udt_process_handle(udt_recv_data.data, udt_recv_data.data_len, udt_recv_data.chl);
        iot_free(udt_recv_data.data);
    }
}
...
{
	/* 参数检测 */
	if ((tx_data->pkg_total == 0) || (tx_data->data == NULL)) {
        return -1;
    }

	/* 安装协议 */
	int len = sizeof(udt_protocol_t);
	udt_protocol_t *udt_prot = (udt_protocol_t *)buffer;//串行载荷协议报文
	udt_prot->mark = UDT_PROT_MARK;
	udt_prot->version = UDT_PROT_VER;
	udt_prot->scene = UDT_PROT_SCENE;
	udt_prot->req_id = 0;
	...
	udt_prot->start_pos = UDT_SWAP32(tx_data->start_pos);
	udt_prot->data_size = UDT_SWAP32(tx_data->data_size);
	memcpy(&buffer[len], tx_data->data, tx_data->data_size);	// 拷贝数据到本地缓存
	len += tx_data->data_size;
	uint16_t crc_md = crc16_modbus_calc(buffer, len);			// 计算modbus crc
	buffer[len++] = crc_md;
	buffer[len++] = crc_md >> 8;
