...
uint16_t dev_modbus_other_inv_online_cnt(void)
{
    uint16_t online_cnt = 0;

	for (int i = 0; i < NET_WIFI_INVBAT_POINT_MINI; i++)
	{
		if(g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn)
		{
			ESP_LOGI(TAG,"dev_modbus_other_inv_online_cnt i:%d iot_sn:%llu",i,g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn);
			if(wlcc_dev_online_check(SN_TYPE_COMMON_INVERTER_WIRELESS, g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn))
			{
				online_cnt++;
			}
		}
	}
	ESP_LOGI(TAG,"online_cnt:%u",online_cnt);
    return online_cnt;
}
...
void dev_modbus_other_inv_update(uint16_t dev_type, uint64_t dev_sn, uint32_t online_time)
{
	...
	for (i = 0; i < NET_WIFI_INVBAT_POINT_MINI; i++)
	{
		...
		if((0 == dev_type_num) && (dev_sn == g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn))
		{
			if(NET_WIFI_INVBAT_POINT_MINI == exist_index){
				exist_index = i;
			}
		}
		else if(0 == g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn)
		{
		...
		{
			last_online_time_tmp = wlcc_dev_online_time_get(dev_type_num, g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn);
			if(0 == wlcc_dev_online_check(dev_type_num, g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn))
			{
		...
		if((dev_type_num == dev_type)
			&& (dev_sn == g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn))
		{
	...
	{
		g_other_rd.other_inv[exist_index].mod_reg11000_IOT_info.iot_sn = dev_sn;
	...
	{
		g_other_rd.other_inv[empty_index].mod_reg11000_IOT_info.iot_sn = dev_sn;
		SN_TYPE_NUM_TO_ASCII(dev_type, pTypeAscii, TYPE_SIZE);
	...
	{
		g_other_rd.other_inv[offline_index].mod_reg11000_IOT_info.iot_sn = dev_sn;
		SN_TYPE_NUM_TO_ASCII(dev_type, pTypeAscii, TYPE_SIZE);
...
int dev_modbus_other_inv_slave_addr_get(uint16_t dev_type, uint64_t dev_sn)
{
	...
	for (i = 0; i < NET_WIFI_INVBAT_POINT_MINI; i++)
    {
        ESP_LOGI(TAG, "modbus[%d] SN:%llu", i, g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn);
		if((0 != dev_sn) && (dev_sn == g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn))
        {
			modbus_slave_addr = i;
			break;
		}
...
uint16_t vLookupOtherInvDataMaxLength(uint16_t regStartAddr)
{
	uint16_t regMaxLen = 0;

	if ((regStartAddr >= MOD_REG_START_ADDR_00000) && (regStartAddr <= (MOD_REG_START_ADDR_00000 + MOD_REG_LEN_00000))){
		regMaxLen = MOD_REG_START_ADDR_00000 + MOD_REG_LEN_00000 - regStartAddr;
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_00100) && (regStartAddr <= (MOD_REG_START_ADDR_00100 + MOD_REG_LEN_00100))){
		regMaxLen = MOD_REG_START_ADDR_00100 + MOD_REG_LEN_00100 - regStartAddr;
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_01100) && (regStartAddr <= (MOD_REG_START_ADDR_01100 + MOD_REG_LEN_01100))){
		regMaxLen = MOD_REG_START_ADDR_01100 + MOD_REG_LEN_01100 - regStartAddr;
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_01300) && (regStartAddr <= (MOD_REG_START_ADDR_01300 + MOD_REG_LEN_01300))){
		regMaxLen = MOD_REG_START_ADDR_01300 + MOD_REG_LEN_01300 - regStartAddr;
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_01400) && (regStartAddr <= (MOD_REG_START_ADDR_01400 + MOD_REG_LEN_01400))){
		regMaxLen = MOD_REG_START_ADDR_01400 + MOD_REG_LEN_01400 - regStartAddr;
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_02000) && (regStartAddr <= (MOD_REG_START_ADDR_02000 + MOD_REG_LEN_02000))){
		regMaxLen = MOD_REG_START_ADDR_02000 + MOD_REG_LEN_02000 - regStartAddr;	
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_02200) && (regStartAddr <= (MOD_REG_START_ADDR_02200 + MOD_REG_LEN_02200))){
		regMaxLen = MOD_REG_START_ADDR_02200 + MOD_REG_LEN_02200 - regStartAddr;	
	}
	else if ((regStartAddr >= MOD_REG_START_ADDR_06000) && (regStartAddr <= (MOD_REG_START_ADDR_06000 + MOD_REG_LEN_06000))){
		regMaxLen = MOD_REG_START_ADDR_06000 + MOD_REG_LEN_06000 - regStartAddr;
	}
...
const uint16_t* vLookupDataTab_from_other_inv( uint16_t dev_type, uint64_t dev_sn, uint16_t regAddr, uint16_t regNum, bool is_write)
{
	...
	ESP_LOGI(TAG, "vLookupDataTab_from_other_inv:slave_addr:%d	dev_type:%u,dev_sn:%llu,regAddr:%u,regNum:%u,is_write:%d",slave_addr,dev_type,dev_sn,regAddr,regNum,is_write);
	...
	else if ((regAddr >= MOD_REG_START_ADDR_01500) && ((regAddr + regNum) <= (MOD_REG_START_ADDR_01500 + MOD_REG_LEN_01500)))
	{
		start = MOD_REG_START_ADDR_01500;	  /* 可读 */

		if(true == is_write){//write
			reg_ptr = (const uint16_t*)&g_other_wr.other_inv.mod_reg01500_Inv_inv;
		}
		else { //read
			reg_ptr = (const uint16_t*)&g_other_rd.other_inv[slave_addr].mod_reg01500_Inv_inv;	
		}		
	}
	...
	else if ((regAddr >= MOD_REG_START_ADDR_02200) && ((regAddr + regNum) <= (MOD_REG_START_ADDR_02200 + MOD_REG_LEN_02200)))
	{
		start = MOD_REG_START_ADDR_02200;	  /* 可读 */

		if(true == is_write){//write
			reg_ptr = (const uint16_t*)&g_other_wr.other_inv.mod_reg02200_Inv_advance_set;
		}
		else //read
		{
			ESP_LOGI(TAG,"get in mod_reg02200_Inv_advance_set read from other inv");
			reg_ptr = (const uint16_t*)&g_other_rd.other_inv[slave_addr].mod_reg02200_Inv_advance_set;	
		}			
	}
...
//		}			
//	}
	else if ((regAddr >= MOD_REG_START_ADDR_06000) && ((regAddr + regNum) <= (MOD_REG_START_ADDR_06000 + MOD_REG_LEN_06000)))
	{
		start = MOD_REG_START_ADDR_06000;	  /* 可读 */

		if(true == is_write){//write
			reg_ptr = (const uint16_t*)&g_other_wr.other_inv.mod_reg06000_Pack_sum;
		}
		else //read
		{
			ESP_LOGI(TAG,"get in mod_reg06000_Pack_sum read from other inv");
			reg_ptr = (const uint16_t*)&g_other_rd.other_inv[slave_addr].mod_reg06000_Pack_sum;	
		}			
	}
	else if ((regAddr >= MOD_REG_START_ADDR_11000) && ((regAddr + regNum) <= (MOD_REG_START_ADDR_11000 + MOD_REG_LEN_11000)))
	{
		start = MOD_REG_START_ADDR_11000;	  /* 可读 */

		if(true == is_write){//write
			reg_ptr = (const uint16_t*)&g_other_wr.other_inv.mod_reg11000_IOT_info;
		}
		else //read
		{
			ESP_LOGI(TAG,"get in mod_reg06000_Pack_sum read from other inv");
			reg_ptr = (const uint16_t*)&g_other_rd.other_inv[slave_addr].mod_reg11000_IOT_info;	
		}			
	}
...
void dev_modbus_other_inv_data_summary(void)
{
	...
	for (i = 0; i < NET_WIFI_INVBAT_POINT_MINI; i++)
	{
		if(g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn)
		{
			...
			if(dev_type && wlcc_dev_online_check(dev_type, g_other_rd.other_inv[i].mod_reg11000_IOT_info.iot_sn))
			{
