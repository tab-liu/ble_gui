...
/*relay控制任务周期*/
#define RELAY_CTRL_TASK_PERIOD  1000

/**********************************************************************************/

//#define RELAY_CTRL_ENABLE_BY_TIME   // 智能控制-定时
//#define RELAY_CTRL_ENABLE_BY_DELAY  // 智能控制-延时
#define RELAY_CTRL_ENABLE_BY_SOC    // 智能控制-SOC
#define RELAY_CTRL_ENABLE_BY_PLAN	// 智能控制-预约备电
...
/*TOU智能控制生效状态*/
static iot_smart_ctrl_struct PowerRelay_SocCtrl_Enable_Status = {0};
static iot_smart_ctrl_struct PowerRelay_DelayCtrl_Enable_Status = {0};
static iot_smart_ctrl_struct PowerRelay_TimeCtrl_Enable_Status = {0};
...
/*单寄存器写入参数*/
extern void Single_Reg_Set_to_IoT(uint8_t SlaveAddress, uint16_t RegAddress, uint16_t value); 
        ...
        for (uint8_t i = 0; i < 8; i++) {
            ...
            if (time_slot->start_time != 0 && time_slot->end_time != 0 && 
                ...
                if (current_time >= time_slot->start_time && current_time <= time_slot->end_time) {
                    // 获取对应时间段的充放电状态 (每个status占用2位)
                    uint8_t status = (Panel_Set->Chg_Plan_Time_En1.all >> (i * 2)) & 0x03;
                    
                    // 仅处理充电状态，放电暂不处理
                    if (status == 1) {  // 1-充电
                        backup_active = 1;
                        ESP_LOGI(RELAY_TAG, "Plan backup power active for time slot %d", i+1);
                        break;  // 找到活动时间段后退出循环
                    }
                }
            }
...
{   
    ...
    if(New_Relay_On_Off_Set != NULL)
    {
        for ( uint8_t i = 0 ; i < RELAY_CTRL_REAL_NUM; i++ )
        {
            uint8_t PowerRelay_Set = GET_POWER_RELAY_STATE(New_Relay_On_Off_Set->all, i);
            ...
            if ( 0 != PowerRelay_Set )
            {
                ...
                switch ( index )
                {
                    case RELAY_INDEX_DC_LOAD1 :
                        set_value = (PowerRelay_Set == 1) ? 3 : ((PowerRelay_Set == 2) ? 2 : 0xFF);
                        if(set_value != 0xFF) Single_Reg_Set_to_IoT(MD_SUMMARY_ADDR, 2012, set_value);
                        break;
                    case RELAY_INDEX_AC_LOAD1 :
                        set_value = (PowerRelay_Set == 1) ? 3 : ((PowerRelay_Set == 2) ? 2 : 0xFF);
                        if(set_value != 0xFF) Single_Reg_Set_to_IoT(MD_SUMMARY_ADDR, 2011, set_value);
                        break;
                    default:
...
/*------------------------------------------------------------------------------
 Function: Relay_Ctrl_Check_From_Soc
 -----------------------------------------------------------------------------*/
/**
  * @brief      SOC智能控制有效检查
  * @param[in]  void  
  ...
  * @return     static void
  */
static void Relay_Ctrl_Check_From_Soc(void)
{   
    MOD_STRUCT_reg19000 *Panel_Set = &RelaySetData.Relay_info_t.PowerRelay_SmartSet01;
    ...
    for ( uint8_t i = 0 ; i < RELAY_CTRL_REAL_NUM ; i++ )
    {
        ...
        /*检查是否存在有效设置*/
        if((PowerRelay_Soc.bit.CmdOnSoc <= 100) || (PowerRelay_Soc.bit.CmdOffSoc <= 100))
        {
            /*闭合SOC和断开SOC不同，设置有效*/
            if ( PowerRelay_Soc.bit.CmdOnSoc != PowerRelay_Soc.bit.CmdOffSoc ) {
                /*该路设置有效*/
                SET_BIT_BY_ENABLE_STATUS(PowerRelay_SocCtrl_Enable_Status.all, i, 1);
                continue;
            }
        }

        /*该路设置无效*/
        SET_BIT_BY_ENABLE_STATUS(PowerRelay_SocCtrl_Enable_Status.all, i, 0);
    }
}

/*------------------------------------------------------------------------------
 Function: Relay_Ctrl_Check_From_Time
 -----------------------------------------------------------------------------*/
/**
  * @brief      定时智能控制有效检查
  * @param[in]  void  
  ...
  * @return     static void
  */
static void Relay_Ctrl_Check_From_Time(void)
{   
    MOD_STRUCT_reg19300 *Panel_Set = &RelaySetData.Relay_info_t.PowerRelay_SmartSet03;
    ...
    for ( uint8_t i = 0 ; i < RELAY_CTRL_REAL_NUM; i++ )
    {
        ...
        {  
            for ( uint8_t j = 0 ; j < RELAY_TIME_CTRL_NODE_NUM ; j++ )
            {
                ...
                /*bit6~0:周六~周日(六(bit6)，五，四，三，二，一，日(bit0))的自定义模式*/
                if ( 0 != Panel_Set->PowerRelayTime_Set_General[label_index].time_label.bit.week )
                {
                    /*该路设置有效*/
                    SET_BIT_BY_ENABLE_STATUS(PowerRelay_TimeCtrl_Enable_Status.all, i, 1);
        ...
        /*该路设置无效*/
        SET_BIT_BY_ENABLE_STATUS(PowerRelay_TimeCtrl_Enable_Status.all, i, 0);
...
/*------------------------------------------------------------------------------
 Function: Relay_Ctrl_Check_From_Delay
 -----------------------------------------------------------------------------*/
/**
  * @brief      延时智能控制有效检查
  * @param[in]  void  
  ...
  * @return     static void
  */
static void Relay_Ctrl_Check_From_Delay(void)
{   
    MOD_STRUCT_reg19100 *Panel_Set = &RelaySetData.Relay_info_t.PowerRelay_SmartSet02;
    ...
    for ( uint8_t i = 0 ; i < RELAY_CTRL_REAL_NUM; i++ )
    {
        ...
        if ( 1 == GET_POWER_RELAY_STATE(Panel_Set->GeneralPowerRelay_Delay_Time_En.all, i))
        {  
            if ( 0 != GET_POWER_RELAY_STATE(Panel_Set->GeneralPowerRelayDelay_Action.all, i))
            {
                /*该路设置有效*/
                SET_BIT_BY_ENABLE_STATUS(PowerRelay_DelayCtrl_Enable_Status.all, i, 1);
                continue;
            }
        }

        /*该路设置无效*/
        SET_BIT_BY_ENABLE_STATUS(PowerRelay_DelayCtrl_Enable_Status.all, i, 0);
    }   
}

/*------------------------------------------------------------------------------
 Function: IOT_SmartCtrl_Enable_Status_Check
 -----------------------------------------------------------------------------*/
/**
  * @brief      智能控制有效检查
  * @param[in]  void  
  ...
  * @return     static void
  */
static void IOT_SmartCtrl_Enable_Status_Check(void)
{
#ifdef RELAY_CTRL_ENABLE_BY_SOC        
    /*SOC智能控制有效检查*/
    Relay_Ctrl_Check_From_Soc();
#endif

#ifdef RELAY_CTRL_ENABLE_BY_DELAY
    /*延时智能控制有效检查*/
    Relay_Ctrl_Check_From_Delay();
#endif

#ifdef RELAY_CTRL_ENABLE_BY_TIME
    /*定时智能控制有效检查*/
    Relay_Ctrl_Check_From_Time();
#endif
}
...
void Relay_Ctrl_Factory_Parameter03_Update(void)
{
    ...
    for ( uint8_t i = 0 ; i < RELAY_TIME_CTRL_NODE_NUM ; i++ ) {
        Panel_Set->PowerRelayTime_Set_General[time_label_index++].time_label.bit.index = RELAY_INDEX_DC_LOAD1;
    }
    
    /*固定时间点标签，不可更改*/
    for ( uint8_t i = 0 ; i < RELAY_TIME_CTRL_NODE_NUM ; i++ ) {
        Panel_Set->PowerRelayTime_Set_General[time_label_index++].time_label.bit.index = RELAY_INDEX_AC_LOAD1;
    }
}
...
uint32_t Relay_SocCtrl_Enable_Status_Get(void) {
    return PowerRelay_SocCtrl_Enable_Status.all;
}
...
uint32_t Relay_DelayCtrl_Enable_Status_Get(void) {
    return PowerRelay_DelayCtrl_Enable_Status.all;
}
...
uint32_t Relay_TimeCtrl_Enable_Status_Get(void) {
    return PowerRelay_TimeCtrl_Enable_Status.all;
}
...
void Relay_Ctrl_Main_Task(void)
{
	...
	if((now_time - pre_time) >= RELAY_CTRL_TASK_PERIOD)
	{
	    pre_time = now_time;
        ...
        /*智能控制有效检查*/
        IOT_SmartCtrl_Enable_Status_Check();
...
#ifdef RELAY_CTRL_ENABLE_BY_PLAN
        /*基于预约备电的控制任务*/
