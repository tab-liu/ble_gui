...
#define MONITOR_UNIT_POWER   "W"
#define MONITOR_UNIT_ENERGY  "kwh"
#define MONITOR_UNIT_CURRENT "A"
#define MONITOR_UNIT_VOLTAGE "V"
#define MONITOR_UNIT_PERCENT "%"

static httpd_handle_t web_server_handle = NULL;//ws服务器唯一句柄
static QueueHandle_t  ws_server_recv_queue = NULL;//收到的消息传给任务处理

/*此处只是管理ws socket server发送时的对象，以确保多客户端连接的时候都能收到数据，并不能限制HTTP请求*/
USE_EXT_RAM_BSS static int ws_client_list[WS_CLIENT_QUANTITY_ASTRICT];//客户端套接字列表
static int ws_client_num = 0;   //实际连接数量

USE_EXT_RAM_BSS ws_svr_msg_t ws_server_msg[WS_SERVER_MSG_MAX];

#define reg_addr_rd top_modbus_rd.Inv[reals.Addr_can_self]
#define reg_addr_rd_total top_modbus_rd.Inv[INV_MAX_NUM]
#define reg_addr_wr top_modbus_wr.Inv
#define iot_config_param IotSetData
...
{
    // 分配内存: 每个字节需2字符 + 终止符
    char *str = (char*)iot_calloc(len * 2 + 1);
    if (str == NULL)
    {
...
static int16_t ws_server_multipart_parse(httpd_req_t *req, char **file_content, int *file_len, int *total_chunks)
{
    if (req == NULL)
    {
        ESP_LOGE(TAG, "ws_server_multipart_parse req is NULL");
        return -1;
    }
    ...
    {
        content_type = (char*)iot_calloc(content_type_len);
    ...
    // 读取body
    char *buf = (char*)iot_calloc(req->content_len + 2);
    if (!buf)
    {
        free(boundary);
        ESP_LOGE(TAG, "malloc failed");
    ...
    char *boundary_full = (char*)iot_calloc(content_type_len + 2);
    if (!boundary_full)
    {
        free(buf);
        free(boundary);
    ...
    while (part)
    {
        ...
        if (name_pos && name_pos < header_end)
        {
            ...
            if (name_end && name_end < header_end)
            {
                ...
                char *value = (char*)iot_calloc(value_len + 1);
                if (value)
                {
...
static int16_t cert_integrity_check(int16_t file_type, char *buffer, int buffer_len)
{
    ...
    if(file_type == 0)
    {
        ...
        mbedtls_pk_init(&pk);
        mbedtls_ctr_drbg_context *ctr_drbg_ptr = (mbedtls_ctr_drbg_context *)iot_calloc(sizeof(mbedtls_ctr_drbg_context)+1);
    ...
    else
    {
        ...
        ESP_LOGI(TAG,"CERTIFICATE check len:%d, data:[%s]", buffer_len+1, buffer);
        esp_log_buffer_hex(TAG, buffer, buffer_len + 1);
...
static esp_err_t ws_server_recv_data(httpd_req_t *req)
{
    ...
    ws_recv_msg.data = (char *)iot_calloc(ws_pkt.len + 1);
    ...
    if (ws_server_recv_queue && xQueueSend(ws_server_recv_queue, &ws_recv_msg, pdMS_TO_TICKS(1)) == pdPASS)
    {
        ret = ESP_OK;
    }
    else
...
static esp_err_t ws_server_update_ca_cert_file(httpd_req_t *req)
{
    ...
    // print_partition_info(MD_TCP_SERVER_CA_AREA, NORMAL_INFO);
...
static esp_err_t ws_server_update_server_cert_file(httpd_req_t *req)
{
    if (req == NULL)
    {
        ESP_LOGE(TAG, "ws_server_update_server_cert_file req is NULL");
        return ESP_FAIL;
    }
    ...
    if (ret <= 0)
    {
        ESP_LOGE(TAG, "Server cert file error!");
        httpd_resp_set_status(req, "401 File Error");
        httpd_resp_send(req, "File error", HTTPD_RESP_USE_STRLEN);
        return ESP_FAIL;
    }
    ...
    // ESP_LOGW(TAG, "update Server cert file: ");
    // print_partition_info(MD_TCP_SERVER_CERT_AREA, NORMAL_INFO);
...
static esp_err_t ws_server_update_server_key_file(httpd_req_t *req)
{
    if (req == NULL)
    {
        ESP_LOGE(TAG, "ws_server_update_server_key_file req is NULL");
        return ESP_FAIL;
    }
    ...
    if (ret <= 0)
    {
        ESP_LOGE(TAG, "Server key file error!");
        httpd_resp_set_status(req, "401 File Error");
        httpd_resp_send(req, "File error", HTTPD_RESP_USE_STRLEN);
        return ESP_FAIL;
    }
    ...
    // ESP_LOGW(TAG, "update Server key file: ");
    // print_partition_info(MD_TCP_SERVER_PRIVATE_AREA, NORMAL_INFO);
...
static void ws_server_send(const uint8_t *data ,uint32_t len , int client_socket)
{
    ...
    send_buffer.data = (char *)iot_calloc(len + 1);
...
#define WS_SVR_WIFI_STA1_SET_RESP_TYPE   "setWiFiSta1Rsp"
    ...
    reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_auth = authmode;
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg12000_IOT_set.wifi_sta_auth = authmode;
    reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_auth = authmode;

    memset(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_ssid, 0x00, sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_ssid));
    memcpy(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_ssid, ssid,
                MIN(strlen(ssid), sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_ssid)));
    ...
    memset(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_ssid, 0x00, sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_ssid));
    memcpy(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_ssid, ssid,
                MIN(strlen(ssid), sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_ssid)));

    if (strlen(pwd) > 0)
    {
        memset(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password, 0x00, sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password));
        memcpy(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password, pwd,
                MIN(strlen(pwd), sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password)));
        // memcpy(Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg12000_IOT_set.wifi_sta_password, pwd,
        //         MIN(strlen(pwd), sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password)));
        memset(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_password, 0x00, sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password));
        memcpy(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_password, pwd,
                MIN(strlen(pwd), sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_password)));
    }
    else
    {
        memset(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password, 0, sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_sta_password));
        ...
        memset(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_password, 0, sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_sta_password));
    }

    reg_addr_wr.mod_reg12000_IOT_set.on_off.bit.wifi_enable = enable == true ? 1 : 2; //WiFi sta1 使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg12000_IOT_set.on_off.bit.wifi_enable = enable == true ? 1 : 2; //WiFi sta1 使能
    reg_addr_rd.mod_reg12000_IOT_set.on_off.bit.wifi_enable = enable == true ? 1 : 2; //WiFi sta1 使能

    // reg_addr_wr.mod_reg13600_open.wifi_mul_sta_en.sta1_en = enable == true ? 1 : 2; //WiFi sta1 使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_mul_sta_en.sta1_en = enable == true ? 1 : 2; //WiFi sta1 使能
    // reg_addr_rd.mod_reg13600_open.wifi_mul_sta_en.sta1_en = enable == true ? 1 : 2; //WiFi sta1 使能

    reg_addr_wr.mod_reg13600_open.wifi_mul_sta_en.sta1_static_ip_en = static_ip_en == true ? 1 : 2; //WiFi sta1 静态IP使能
    ...
    reg_addr_rd.mod_reg13600_open.wifi_mul_sta_en.sta1_static_ip_en = static_ip_en == true ? 1 : 2; //WiFi sta1 静态IP使能

    if (true == static_ip_en)
    {
        ret = inet_aton(ip, &addr_int);
        if (ret == 0)
        {
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta1_ip = addr_int; //WiFi sta1 ip地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta1_ip = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta1_ip = addr_int;

        ret = inet_aton(mask, &addr_int);
        if (ret == 0)
        {
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta1_mask = addr_int; //WiFi sta1 网络掩码地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta1_mask = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta1_mask = addr_int;

        ret = inet_aton(gw, &addr_int);
        if (ret == 0)
        {
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta1_gw = addr_int; //WiFi sta1 网关地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta1_gw = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta1_gw = addr_int;

        ret = inet_aton(dns1, &addr_int);
        if (ret == 0)
        {
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta1_dns1 = addr_int; //WiFi sta1 网关地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta1_dns1 = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta1_dns1 = addr_int;

        if (dns2 != NULL)
        {
            ret = inet_aton(dns2, &addr_int);
            if (ret == 0)
            {
            ...
            reg_addr_wr.mod_reg13600_open.wifi_sta1_dns2 = addr_int; //WiFi sta1 网关地址
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta1_dns2 = addr_int;
            reg_addr_rd.mod_reg13600_open.wifi_sta1_dns2 = addr_int;
        }
        else
        {
            reg_addr_wr.mod_reg13600_open.wifi_sta1_dns2 = 0; //WiFi sta1 DNS2 地址
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta1_dns2 = 0;
            reg_addr_rd.mod_reg13600_open.wifi_sta1_dns2 = 0;
        }
...
#define WS_SVR_WIFI_STA2_SET_RESP_TYPE   "setWiFiSta2Rsp"
    ...
    reg_addr_wr.mod_reg13600_open.wifi_sta2_auth = authmode; //WiFi sta1 认证方式
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_auth = authmode;
    reg_addr_rd.mod_reg13600_open.wifi_sta2_auth = authmode;

    memset(reg_addr_wr.mod_reg13600_open.wifi_sta2_ssid, 0x00, sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta2_ssid));
    memcpy(reg_addr_wr.mod_reg13600_open.wifi_sta2_ssid, ssid,
            MIN(strlen(ssid), sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta2_ssid)));
    ...
    memset(reg_addr_rd.mod_reg13600_open.wifi_sta2_ssid, 0x00, sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta2_ssid));
    memcpy(reg_addr_rd.mod_reg13600_open.wifi_sta2_ssid, ssid,
            MIN(strlen(ssid), sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta2_ssid)));

    if (strlen(pwd) > 0)
    {
        memset(reg_addr_wr.mod_reg13600_open.wifi_sta2_password, 0x00, sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta2_password));
        memcpy(reg_addr_wr.mod_reg13600_open.wifi_sta2_password, pwd,
                MIN(strlen(pwd), sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta2_password)));
        ...
        memset(reg_addr_rd.mod_reg13600_open.wifi_sta2_password, 0x00, sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta2_password));
        memcpy(reg_addr_rd.mod_reg13600_open.wifi_sta2_password, pwd,
                MIN(strlen(pwd), sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta2_password)));
    }
    else
    {
        memset(reg_addr_wr.mod_reg13600_open.wifi_sta2_password, 0x0, sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta2_password));
        ...
        memset(reg_addr_rd.mod_reg13600_open.wifi_sta2_password, 0x0, sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta2_password));
    }

    reg_addr_wr.mod_reg13600_open.wifi_mul_sta_en.sta2_en = enable == true? 1 : 2; //WiFi sta1 使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_mul_sta_en.sta2_en = enable == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.wifi_mul_sta_en.sta2_en = enable == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.wifi_mul_sta_en.sta2_static_ip_en = static_ip_en == true? 1 : 2; //WiFi sta1 静态IP使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_mul_sta_en.sta2_static_ip_en = static_ip_en == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.wifi_mul_sta_en.sta2_static_ip_en = static_ip_en == true? 1 : 2;
    ...
    if (true == static_ip_en)
    {
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta2_ip = addr_int; //WiFi sta1 ip地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_ip = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta2_ip = addr_int;
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta2_mask = addr_int; //WiFi sta1 网络掩码地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_mask = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta2_mask = addr_int;
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta2_gw = addr_int; //WiFi sta1 网关地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_gw = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta2_gw = addr_int;
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta2_dns1 = addr_int; //WiFi sta1 网关地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_dns1 = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta2_dns1 = addr_int;
        ...
        if (dns2 != NULL)
        {
            ...
            reg_addr_wr.mod_reg13600_open.wifi_sta2_dns2 = addr_int; //WiFi sta1 网关地址
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_dns2 = addr_int;
            reg_addr_rd.mod_reg13600_open.wifi_sta2_dns2 = addr_int;
        }
        else
        {
            reg_addr_wr.mod_reg13600_open.wifi_sta2_dns2 = 0; //WiFi sta1 DNS2 地址
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta2_dns2 = 0;
            reg_addr_rd.mod_reg13600_open.wifi_sta2_dns2 = 0;
        }
...
#define WS_SVR_WIFI_STA3_SET_RESP_TYPE   "setWiFiSta3Rsp"
    ...
    reg_addr_wr.mod_reg13600_open.wifi_sta3_auth = authmode; //WiFi sta1 认证方式
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_auth = authmode;
    reg_addr_rd.mod_reg13600_open.wifi_sta3_auth = authmode;

    memset(reg_addr_wr.mod_reg13600_open.wifi_sta3_ssid, 0x00, sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta3_ssid));
    memcpy(reg_addr_wr.mod_reg13600_open.wifi_sta3_ssid, ssid,
            MIN(strlen(ssid), sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta3_ssid)));
    ...
    memset(reg_addr_rd.mod_reg13600_open.wifi_sta3_ssid, 0x00, sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta3_ssid));
    memcpy(reg_addr_rd.mod_reg13600_open.wifi_sta3_ssid, ssid,
            MIN(strlen(ssid), sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta3_ssid)));

    memset(reg_addr_wr.mod_reg13600_open.wifi_sta3_password, 0x00, sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta3_password));
    memcpy(reg_addr_wr.mod_reg13600_open.wifi_sta3_password, pwd,
            MIN(strlen(pwd), sizeof(reg_addr_wr.mod_reg13600_open.wifi_sta3_password)));
    ...
    memset(reg_addr_rd.mod_reg13600_open.wifi_sta3_password, 0x00, sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta3_password));
    memcpy(reg_addr_rd.mod_reg13600_open.wifi_sta3_password, pwd,
            MIN(strlen(pwd), sizeof(reg_addr_rd.mod_reg13600_open.wifi_sta3_password)));

    reg_addr_wr.mod_reg13600_open.wifi_mul_sta_en.sta3_en = enable == true? 1 : 2; //WiFi sta1 使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_mul_sta_en.sta3_en = enable == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.wifi_mul_sta_en.sta3_en = enable == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.wifi_mul_sta_en.sta3_static_ip_en = static_ip_en == true? 1 : 2; //WiFi sta1 静态IP使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_mul_sta_en.sta3_static_ip_en = static_ip_en == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.wifi_mul_sta_en.sta3_static_ip_en = static_ip_en == true? 1 : 2;
    ...
    if (true == static_ip_en)
    {
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta3_ip = addr_int; //WiFi sta1 ip地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_ip = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta3_ip = addr_int;
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta3_mask = addr_int; //WiFi sta1 网络掩码地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_mask = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta3_mask = addr_int;
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta3_gw = addr_int; //WiFi sta1 网关地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_gw = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta3_gw = addr_int;
        ...
        reg_addr_wr.mod_reg13600_open.wifi_sta3_dns1 = addr_int; //WiFi sta1 网关地址
        // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_dns1 = addr_int;
        reg_addr_rd.mod_reg13600_open.wifi_sta3_dns1 = addr_int;
        ...
        if (dns2 != NULL)
        {
            ...
            reg_addr_wr.mod_reg13600_open.wifi_sta3_dns2 = addr_int; //WiFi sta1 网关地址
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_dns2 = addr_int;
            reg_addr_rd.mod_reg13600_open.wifi_sta3_dns2 = addr_int;
        }
        else
        {
            reg_addr_wr.mod_reg13600_open.wifi_sta3_dns2 = 0; //WiFi sta1 DNS2 地址
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.wifi_sta3_dns2 = 0;
            reg_addr_rd.mod_reg13600_open.wifi_sta3_dns2 = 0;
        }
...
#define WS_SVR_WIFI_AP_SET_RESP_TYPE   "setWiFiApRsp"
    ...
    ESP_LOGI(TAG, "Wi-Fi AP enable:%d, SSID:%s, PWD:%s, auth:%d", enable, ssid, pwd, auth);

    reg_addr_wr.mod_reg12000_IOT_set.on_off.bit.wifi_ap_enable = enable == true? 1 : 2;
    ESP_LOGW(TAG, "on-off config: %08lx --> %08lx", reg_addr_rd.mod_reg12000_IOT_set.on_off.all, reg_addr_wr.mod_reg12000_IOT_set.on_off.all);
    reg_addr_rd.mod_reg12000_IOT_set.on_off.bit.wifi_ap_enable = enable == true? 1 : 2;

    memset(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_ssid, 0x00, sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_ssid));
    memcpy(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_ssid, ssid,
            MIN(strlen(ssid), sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_ssid)));
    ...
    memset(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_ssid, 0x00 ,sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_ssid));
    memcpy(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_ssid, ssid,
            MIN(strlen(ssid), sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_ssid)));

    //AP密码长度必须超过8个字符
    if (auth != WIFI_AUTH_OPEN && strlen(pwd) < 8)
    {
        if ( enable == false && 0 == strlen(pwd) )
        {
            // 关闭AP时允许密码为空，视为保留原密码
            memcpy(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password, reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password,
                    sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password));
        }
        else
        {
            ESP_LOGE(TAG, "wifi ap set password length is less than 8");
            return ws_server_msg_response(WS_SVR_WIFI_AP_SET_RESP_TYPE, 5, "Wi-Fi AP password length is less than 8", resp_data);
        }
    }
    else
    {
        if (strlen(pwd) > 0)
        {
            memset(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password, 0x00, sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password));
            memcpy(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password, pwd,
                    MIN(strlen(pwd), sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password)));
            ...
            memset(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password, 0x00 ,sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password));
            memcpy(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password, pwd,
                    MIN(strlen(pwd), sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password)));
        
            reg_addr_wr.mod_reg12000_IOT_set.wifi_ap_auth = auth;
            // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg12000_IOT_set.wifi_ap_auth = auth;
            reg_addr_rd.mod_reg12000_IOT_set.wifi_ap_auth = auth;
        }
        else
        {
            memset(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password, 0x0,sizeof(reg_addr_wr.mod_reg12000_IOT_set.wifi_AP_password));
            ...
            memset(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password, 0x0,sizeof(reg_addr_rd.mod_reg12000_IOT_set.wifi_AP_password));
...
#define WS_SVR_BLE_SVR_SET_RESP_TYPE   "setBleServerRsp"
    ...
    //设置BLE使能: 同时设置g_self_data_WR、Inv和g_self_data
    reg_addr_wr.mod_reg12000_IOT_set.on_off.bit.ble_enable = enable == true? 1 : 2;
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg12000_IOT_set.on_off.bit.ble_enable = enable == true? 1 : 2;
    reg_addr_rd.mod_reg12000_IOT_set.on_off.bit.ble_enable = enable == true? 1 : 2;
    // write_register_handle(MD_SLAVE_ADDR_SUM, 12161, 2); //同步到

    reg_addr_wr.mod_reg13600_open.ble_protocol.adv_en = adv_enable == true? 1 : 2;
    if (strlen(pwd))
    {
        memset((char *)reg_addr_wr.mod_reg00000.app_password,0x00,sizeof((char *)reg_addr_wr.mod_reg00000.app_password));
        memcpy((char *)reg_addr_wr.mod_reg00000.app_password, pwd,
                MIN(strlen(pwd), sizeof(reg_addr_wr.mod_reg00000.app_password)));
        ...
        memset((char *)reg_addr_rd.mod_reg00000.app_password,0x00,sizeof((char *)reg_addr_rd.mod_reg00000.app_password));
        memcpy((char *)reg_addr_rd.mod_reg00000.app_password, pwd,
                MIN(strlen(pwd), sizeof(reg_addr_rd.mod_reg00000.app_password)));
    }
    else
    {
        memset((char *)reg_addr_wr.mod_reg00000.app_password, 0x0, sizeof(reg_addr_wr.mod_reg00000.app_password));
        ...
        memset((char *)reg_addr_rd.mod_reg00000.app_password, 0x0, sizeof(reg_addr_rd.mod_reg00000.app_password));
    }
    // write_register_handle(MD_SLAVE_ADDR_SUM, 7, 3);
    ...
    if (ret <= 0)
    {
        memset(reg_addr_wr.mod_reg13600_open.bles_adv_key, 0x00, sizeof(reg_addr_wr.mod_reg13600_open.bles_adv_key));
    ...
    {
        memset((char *)reg_addr_wr.mod_reg13600_open.bles_adv_key,0x00,sizeof((char *)reg_addr_wr.mod_reg13600_open.bles_adv_key));
        memcpy((char *)reg_addr_wr.mod_reg13600_open.bles_adv_key, ble_adv_key,
                MIN(ret, sizeof(reg_addr_wr.mod_reg13600_open.bles_adv_key)));
    ...
    // write_register_handle(MD_SLAVE_ADDR_SUM, 13600, 11);
...
#define WS_SVR_MQTT_SET_RESP_TYPE   "setMqttRsp"
    ...
    reg_addr_wr.mod_reg13600_open.open_mqtt_enable.enable = enable == true? 1 : 2; //MQTT使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.open_mqtt_enable.enable = enable == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.open_mqtt_enable.enable = enable == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.open_mqtt_enable.crypt_en = crypt_en == true? 1 : 2; //MQTT加密使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.open_mqtt_enable.crypt_en = crypt_en == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.open_mqtt_enable.crypt_en = crypt_en == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.open_mqtt_enable.ctrl_en = ctrl_en == true? 1 : 2; //MQTT控制使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.open_mqtt_enable.ctrl_en = ctrl_en == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.open_mqtt_enable.ctrl_en = ctrl_en == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.open_mqtt_enable.report_en = report_en == true? 1 : 2; //MQTT上报使能
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.open_mqtt_enable.report_en = report_en == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.open_mqtt_enable.report_en = report_en == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.open_mqtt_report_cycle = report_cycle; //MQTT上报周期
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.open_mqtt_report_cycle = report_cycle;
    reg_addr_rd.mod_reg13600_open.open_mqtt_report_cycle = report_cycle;

    memset(reg_addr_wr.mod_reg22000_net_server_2rd.Net_Server_address,0x00,sizeof(reg_addr_wr.mod_reg22000_net_server_2rd.Net_Server_address));
    memcpy(reg_addr_wr.mod_reg22000_net_server_2rd.Net_Server_address, url,
            MIN(strlen(url), sizeof(reg_addr_wr.mod_reg22000_net_server_2rd.Net_Server_address)));
    ...
    memset(reg_addr_rd.mod_reg22000_net_server_2rd.Net_Server_address,0x00,sizeof(reg_addr_rd.mod_reg22000_net_server_2rd.Net_Server_address));
    memcpy(reg_addr_rd.mod_reg22000_net_server_2rd.Net_Server_address, url,
            MIN(strlen(url), sizeof(reg_addr_rd.mod_reg22000_net_server_2rd.Net_Server_address)));
...
#define WS_SVR_MD_TCP_SET_RESP_TYPE   "setModbusTcpRsp"
    ...
    if (crypt_en == true)
    {
        ...
        if( 0 == load_modbus_tcp_cert() ) {
            ESP_LOGE(TAG, "load_modbus_tcp_cert error");
            return ws_server_msg_response(WS_SVR_MD_TCP_SET_RESP_TYPE, 2, "Please upload TLS certificate firstly.", resp_data);
        }
#else
        ESP_LOGE(TAG, "modbus tcp crypt unsupported");
        return ws_server_msg_response(WS_SVR_MD_TCP_SET_RESP_TYPE, 2, "Modbus TCP crypt unsupported", resp_data);
#endif
    }
    ...
    reg_addr_wr.mod_reg13600_open.modbus_tcp_enable.enable = enable == true? 1 : 2;
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.modbus_tcp_enable.enable = enable == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.modbus_tcp_enable.enable = enable == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.modbus_tcp_enable.crypt_en = crypt_en == true? 1 : 2;
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.modbus_tcp_enable.crypt_en = crypt_en == true? 1 : 2;
    reg_addr_rd.mod_reg13600_open.modbus_tcp_enable.crypt_en = crypt_en == true? 1 : 2;

    reg_addr_wr.mod_reg13600_open.modbus_tcp_port = port;
    // Inv[(INV_MAX_NUM*DEV_MAIN_NODE_MAX)].mod_reg13600_open.modbus_tcp_port = port;
    reg_addr_rd.mod_reg13600_open.modbus_tcp_port = port;
...
{
extern ota_cmd_list_t *new_firmware_iot_list;
...
#define WS_SVR_VERSION_GET_RESP_TYPE   "getVersionRsp"
    ...
    if (new_firmware_iot_list)
    {
        ...
        while (current)
        {
            ...
            snprintf(str_tmp, sizeof(str_tmp), "%lu", top_modbus_rd.Inv[reals.Addr_can_self].mod_reg11000_IOT_info.software_ver);
            ...
            ESP_LOGW(TAG, "----- firmware info, model:%s, version:%lu, newVersion:%lu", current->object.model,
                    top_modbus_rd.Inv[reals.Addr_can_self].mod_reg11000_IOT_info.software_ver, current->object.version);
    ...
    {
        cjson_temp = cJSON_CreateObject();
        cJSON_AddStringToObject(cjson_temp, "model", top_modbus_rd.Inv[reals.Addr_can_self].mod_reg11000_IOT_info.iot_type);
        snprintf(str_tmp, sizeof(str_tmp), "%lu", top_modbus_rd.Inv[reals.Addr_can_self].mod_reg11000_IOT_info.software_ver);
        cJSON_AddStringToObject(cjson_temp, "version", str_tmp);
        snprintf(str_tmp, sizeof(str_tmp), "%lu", top_modbus_rd.Inv[reals.Addr_can_self].mod_reg11000_IOT_info.software_ver);
        cJSON_AddStringToObject(cjson_temp, "newVersion", str_tmp);
...
int16_t ws_svr_msg_upgrade_handler(cJSON *cjson_object, uint8_t **resp_data)
{
    ...
    firmware_upgrade_start(model, version, OTA_METHOD_WEBSERVER);

    return ws_server_upgrade_progress(UPGRADE_STATE_DOING, 0, resp_data);
}

static int16_t ws_server_scan_device_add_json(cJSON *cjson_data, uint8_t mfg_id, void *dev_info)
{
    char temp_str[32] = {0};
    cJSON *cjson_dev = cJSON_CreateObject();
    if (cjson_dev == NULL) {
        ESP_LOGE(TAG, "ws_server_msg_response cjson_dev is NULL");
        return -1;
    }
    ...
    if (mfg_id == METER_MFG_ID_BLUETTI)
    {
        POINT_BIND_INFO *point_bind_info = (POINT_BIND_INFO *)dev_info;

        memset(temp_str, 0x00, sizeof(temp_str));
        snprintf(temp_str, sizeof(temp_str), "%llu", (unsigned long long)point_bind_info->SN_64);
        cJSON_AddItemToObject(cjson_dev, "mfg_id", cJSON_CreateNumber(METER_MFG_ID_BLUETTI));
        cJSON_AddItemToObject(cjson_dev, "dev_type", cJSON_CreateNumber(point_bind_info->Dev_Type));
        cJSON_AddItemToObject(cjson_dev, "sn", cJSON_CreateString(temp_str));
        SN_TYPE_NUM_TO_ASCII(point_bind_info->Dev_Type, temp_str, sizeof(temp_str));
        cJSON_AddItemToObject(cjson_dev, "model", cJSON_CreateString(temp_str));

        uint8_t is_bound = 0;
        if (SN_TYPE_S1 == point_bind_info->Dev_Type) {
#ifdef CONFIG_WLCC_PLUG_SUPPLY_ENABLE            
            if (plug_is_exist(point_bind_info->SN_64) >= 0) {
                is_bound = 1;
            }
#endif
        }
        else if (SN_TYPE_METER == point_bind_info->Dev_Type)
        {
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE        
            if (bluetti_meter_is_exist(point_bind_info->SN_64) >= 0) {
                is_bound = 1;
            }

            MOD_STRUCT_Meter *meter = &g_other_rd.Meter[point_bind_info->slave_addr - 1];
            cJSON_AddItemToObject(cjson_dev, "p_sum", cJSON_CreateNumber(meter->meter_info.Total_Active_Power));
            cJSON_AddItemToObject(cjson_dev, "p_a", cJSON_CreateNumber(meter->meter_info.L1_Active_Power));
            cJSON_AddItemToObject(cjson_dev, "p_b", cJSON_CreateNumber(meter->meter_info.L2_Active_Power));
            cJSON_AddItemToObject(cjson_dev, "p_c", cJSON_CreateNumber(meter->meter_info.L3_Active_Power));
            ESP_LOGI(TAG, "meter SN:%llu(%d), slave_addr:%d, p_sum:%f, p_a:%f, p_b:%f, p_c:%f",
                        point_bind_info->SN_64, point_bind_info->Dev_Type, point_bind_info->slave_addr,
                        meter->meter_info.Total_Active_Power, meter->meter_info.L1_Active_Power,
                        meter->meter_info.L2_Active_Power, meter->meter_info.L3_Active_Power);
#endif            
        }

        cJSON_AddItemToObject(cjson_dev, "binded", cJSON_CreateNumber(is_bound));
        cJSON_AddItemToObject(cjson_dev, "slave_addr", cJSON_CreateNumber(point_bind_info->slave_addr));
        cJSON_AddItemToObject(cjson_dev, "online", cJSON_CreateNumber(point_bind_info->state.bit.point_online));

        cJSON_AddItemToArray(cjson_data, cjson_dev);
    }
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE    
    else if (mfg_id == 1 || mfg_id == 2)
    {
        meter_scan_result_t *meter_scan_result = (meter_scan_result_t *)dev_info;
        if(meter_scan_result!=NULL)
        {
            cJSON_AddItemToObject(cjson_dev, "mfg_id", cJSON_CreateNumber(meter_scan_result->mfg_id));
            cJSON_AddItemToObject(cjson_dev, "dev_type", cJSON_CreateNumber(meter_scan_result->dev_type));
            cJSON_AddItemToObject(cjson_dev, "sn", cJSON_CreateString((char *)meter_scan_result->dev_id));

            memset(temp_str, 0x00, sizeof(temp_str));
            if (meter_scan_result->mfg_id == 1)
            {
                snprintf(temp_str, sizeof(temp_str), "%s", "ShellyPro3EM");
            }
            else
            {
                snprintf(temp_str, sizeof(temp_str), "%s", "EcoTracker IR");
            }
            cJSON_AddItemToObject(cjson_dev, "model", cJSON_CreateString(temp_str));

            cJSON_AddItemToObject(cjson_dev, "ip", cJSON_CreateNumber(meter_scan_result->ip));
            cJSON_AddItemToObject(cjson_dev, "port", cJSON_CreateNumber(meter_scan_result->port));
            cJSON_AddItemToObject(cjson_dev, "binded", cJSON_CreateNumber(meter_scan_result->state.bits.bind));
            cJSON_AddItemToArray(cjson_data, cjson_dev);
        }
    }
#endif    
...
int16_t ws_server_scan_device_progress(uint8_t type, uint8_t mfg_id, uint8_t **resp_data)
{
#define WS_SVR_SCAN_DEVICE_RESP_TYPE   "scanDeviceRsp"
    ...
    cJSON_AddItemToObject(cjson_resp, "type", cJSON_CreateString(WS_SVR_SCAN_DEVICE_RESP_TYPE));

    cJSON *cjson_data = cJSON_CreateArray();
    if (cjson_data == NULL)
    {
        cJSON_Delete(cjson_resp);
        ESP_LOGE(TAG, "ws_server_msg_response cjson_data is NULL");
    ...
    {
        int16_t ret = 0;
        POINT_BIND_INFO *point_bind_info = (POINT_BIND_INFO *)iot_calloc(sizeof(POINT_BIND_INFO) * NET_WIFI_MAX_POINT);
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE          
        ret = bluetti_meter_info_report_process(point_bind_info, true);
        if (ret >= 0) {
            dev_cnt += ret;
        }
#endif
...
#ifdef CONFIG_WLCC_PLUG_SUPPLY_ENABLE
        ret = plug_info_report_process(&point_bind_info[dev_cnt], true);
        if (ret >= 0) {
            dev_cnt += ret;
        }
#endif        
        ...
        for (uint8_t i = 0; i < dev_cnt; i++) {
            ws_server_scan_device_add_json(cjson_data, mfg_id, (void *)&point_bind_info[i]);
        }

        iot_free(point_bind_info);
    }
    // Thirty-party devices
    else if (mfg_id == 1 || mfg_id == 2)
    {
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE     
        meter_scan_result_t *meter_scan_result = (meter_scan_result_t *)iot_calloc(sizeof(meter_scan_result_t) * METER_SCAN_MAX_NUM);
        // 目前shelly和everHome仅支持mdns-meter
        if (1 == type)
        {
            // reals.meter_scan_cmd.mfg_id = METER_MFG_ID_SHELLY;
            dev_cnt = meter_scan_result_get(meter_scan_result, METER_SCAN_MAX_NUM);
            ESP_LOGI(TAG, "meter get meter_scan_result_get count: %d, binded:%d, dev_id:%s, mfg_id:%d",
                        dev_cnt,
                        meter_scan_result[0].state.bits.bind,
                        meter_scan_result[0].dev_id, meter_scan_result[0].mfg_id);

            meter_dev_info_t data[METER_SCAN_MAX_NUM] = {0};
            uint8_t binded_cnt = meter_state_get(data, METER_SCAN_MAX_NUM);
            for (int i = 0; i < binded_cnt; i++)
            {
                meter_scan_result[dev_cnt].mfg_id = data[i].mfg_id;
                meter_scan_result[dev_cnt].dev_type = data[i].dev_type;
                strncpy((char *)meter_scan_result[dev_cnt].dev_id, (char *)data[i].dev_id, sizeof(meter_scan_result[dev_cnt].dev_id)-1);
                meter_scan_result[dev_cnt].ip = data[i].ip;
                meter_scan_result[dev_cnt].port = data[i].port;
                meter_scan_result[dev_cnt].state.bits.bind = 1;

                ESP_LOGI(TAG, "meter get state count: %d, binded:%d, dev_id:%s, mfg_id:%d",
                        dev_cnt,
                        meter_scan_result[dev_cnt].state.bits.bind,
                        meter_scan_result[dev_cnt].dev_id, meter_scan_result[dev_cnt].mfg_id);
                dev_cnt++;
            }

            // reals.meter_scan_cmd.mfg_id = METER_MFG_ID_ECOTRACKER;
            // dev_cnt += meter_scan_result_get(&meter_scan_result[dev_cnt], METER_SCAN_MAX_NUM - dev_cnt);
            ESP_LOGI(TAG, "meter scan result count: %d", dev_cnt);
        }

        for (uint8_t i = 0; i < dev_cnt; i++) {
            ws_server_scan_device_add_json(cjson_data, mfg_id, (void *)&meter_scan_result[i]);
        }

        iot_free(meter_scan_result);
#endif        
    }
...
#define WS_SVR_MONITOR_INFO_GET_RESP_TYPE   "monitorInfo"
    ...
    add_monitor_info(cjson_device, 1, "Pack", "Total Soc", str_tmp, "%", "Remaining battery");
    snprintf(str_tmp, sizeof(str_tmp), "%.1f", (double)(reg_addr_rd_total.mod_reg00100_AppPage1.total_voltage / 10.0));//20260518优化电压电流在Webserver的显示,单位转化
    add_monitor_info(cjson_device, 2, "Pack", "Voltage", str_tmp, "V", "Total Battery Voltage");
    snprintf(str_tmp, sizeof(str_tmp), "%.1f", (double)(reg_addr_rd_total.mod_reg00100_AppPage1.total_current / 10.0));
    add_monitor_info(cjson_device, 3, "Pack", "Current", str_tmp, "A", "Total Battery Current");
    if (0 == reg_addr_rd_total.mod_reg00100_AppPage1.chg_status) { // 待机
        add_monitor_info(cjson_device, 4, "Pack", "Status", "Idle", "", "Status of the battery pack"); 
    } else if (1 == reg_addr_rd_total.mod_reg00100_AppPage1.chg_status) { // 充电
        add_monitor_info(cjson_device, 4, "Pack", "Status", "Charging", "", "Status of the battery pack");
        snprintf(str_tmp, sizeof(str_tmp), "%d", reg_addr_rd_total.mod_reg00100_AppPage1.chg_full_time);
        add_monitor_info(cjson_device, 5, "Pack", "Time to full", str_tmp, "min", "Estimated time to full charge");
    } else if (2 == reg_addr_rd_total.mod_reg00100_AppPage1.chg_status) {   // 放电
        add_monitor_info(cjson_device, 4, "Pack", "Status", "Discharging", "", "Status of the battery pack");
        ...
        add_monitor_info(cjson_device, 4, "Pack", "Status", "Unknown", "", "Status of the battery pack");
        snprintf(str_tmp, sizeof(str_tmp), "%d", reg_addr_rd_total.mod_reg00100_AppPage1.dsg_empty_time);
        add_monitor_info(cjson_device, 5, "Pack", "Time to empty", str_tmp, "min", "Estimated discharge time");
    } 
    ...
    //PV
    snprintf(str_tmp, sizeof(str_tmp), "%lu", reg_addr_rd_total.mod_reg01200_Inv_pv.total_chg_power);
    add_monitor_info(cjson_device, 1, "PV", "Total Power", str_tmp, "W", "PV total power");

    for (int i = 0; i < reg_addr_rd_total.mod_reg01200_Inv_pv.pv_number.pv_total; i++)
    {
        if (reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_type == 1) {
            snprintf(str_tmp, sizeof(str_tmp), "CAR");
        } else if (reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_type == 2) {
            snprintf(str_tmp, sizeof(str_tmp), "Adpater");
        } else if (reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_type == 3) {
            snprintf(str_tmp, sizeof(str_tmp), "Other");
        } else if (reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_type == 100) {
            snprintf(str_tmp, sizeof(str_tmp), "DC");
        } else if (reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_type == 101) {
            snprintf(str_tmp, sizeof(str_tmp), "AC");
        } else {
            snprintf(str_tmp, sizeof(str_tmp), "Unknown(%d)", reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_type);
        }
        
        char str_name[32] = {0};
        snprintf(str_name, sizeof(str_name), "PV%d Type", i + 1);
        add_monitor_info(cjson_device, 1, "PV", str_name, str_tmp, "", str_name);
        //PV Power
        snprintf(str_name, sizeof(str_name), "PV%d Power", i + 1);
        snprintf(str_tmp, sizeof(str_tmp), "%d", reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_power);
        add_monitor_info(cjson_device, 1, "PV", str_name, str_tmp, "W", str_name);
        // PV Voltage
        snprintf(str_name, sizeof(str_name), "PV%d Voltage", i + 1);
        snprintf(str_tmp, sizeof(str_tmp), "%.1f", (double)(reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_voltage / 10.0));
        add_monitor_info(cjson_device, 1, "PV", str_name, str_tmp, "V", str_name);
        // PV Current
        snprintf(str_name, sizeof(str_name), "PV%d Current", i + 1);
        snprintf(str_tmp, sizeof(str_tmp), "%.1f", (double)(reg_addr_rd_total.mod_reg01200_Inv_pv.pv_detail[i].input_current / 10.0));
        add_monitor_info(cjson_device, 1, "PV", str_name, str_tmp, "A", str_name);
    }
    ...
    *resp_data = (uint8_t *)cJSON_PrintUnformatted(cjson_resp);
    ...
    if (resp_data == NULL)
    {
        ESP_LOGE(TAG, "ws server device info resp_data is NULL");
        ret = -4;
        goto err;
    }
...
#define WS_SVR_CONFIG_GET_RESP_TYPE   "getConfigRsp"
    ...
    cJSON_AddItemToObject(cjson_temp, "enable", cJSON_CreateBool(iot_config_param.dev_info_t.on_off.bit.wifi_ap_enable == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "ssid", cJSON_CreateString((char *)iot_config_param.dev_info_t.wifi_AP_ssid));
    // cJSON_AddItemToObject(cjson_temp, "password", cJSON_CreateString(iot_config_param.dev_info_t.wifi_AP_password));
    cJSON_AddItemToObject(cjson_temp, "authmode", cJSON_CreateNumber(iot_config_param.dev_info_t.wifi_ap_auth));
    ...
    cJSON_AddItemToObject(cjson_temp, "enable", cJSON_CreateBool(iot_config_param.dev_info_t.on_off.bit.wifi_enable == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "ssid", cJSON_CreateString((char *)iot_config_param.dev_info_t.wifi_sta_ssid));
    // cJSON_AddItemToObject(cjson_temp, "password", cJSON_CreateString(iot_config_param.dev_info_t.wifi_sta_password));
    cJSON_AddItemToObject(cjson_temp, "authmode", cJSON_CreateNumber(iot_config_param.dev_info_t.wifi_sta_auth));
    cJSON_AddItemToObject(cjson_temp, "static_ip_enable", cJSON_CreateBool(iot_config_param.dev_info_t.wifi_mul_sta_en.sta1_static_ip_en == 1? true : false));
    if (1 == iot_config_param.dev_info_t.wifi_mul_sta_en.sta1_static_ip_en)
    {
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta1_ip));
        cJSON_AddItemToObject(cjson_temp, "ip", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta1_gw, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta1_gw));
        cJSON_AddItemToObject(cjson_temp, "gateway", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta1_mask, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta1_mask));
        cJSON_AddItemToObject(cjson_temp, "netmask", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta1_dns1, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta1_dns1));
        cJSON_AddItemToObject(cjson_temp, "dns1", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta1_dns2, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta1_dns2));
    ...
    cJSON_AddItemToObject(cjson_temp, "enable", cJSON_CreateBool(iot_config_param.dev_info_t.wifi_mul_sta_en.sta2_en == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "ssid", cJSON_CreateString((char *)iot_config_param.dev_info_t.wifi_sta2_ssid));
    // cJSON_AddItemToObject(cjson_temp, "password", cJSON_CreateString((char *)iot_config_param.dev_info_t.wifi_sta2_password));
    cJSON_AddItemToObject(cjson_temp, "authmode", cJSON_CreateNumber(iot_config_param.dev_info_t.wifi_sta2_auth));
    cJSON_AddItemToObject(cjson_temp, "static_ip_enable", cJSON_CreateBool(iot_config_param.dev_info_t.wifi_mul_sta_en.sta2_static_ip_en == 1? true : false));
    if (1 == iot_config_param.dev_info_t.wifi_mul_sta_en.sta2_static_ip_en)
    {
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta2_ip, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta2_ip));
        cJSON_AddItemToObject(cjson_temp, "ip", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta2_gw, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta2_gw));
        cJSON_AddItemToObject(cjson_temp, "gateway", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta2_mask, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta2_mask));
        cJSON_AddItemToObject(cjson_temp, "netmask", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta2_dns1, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta2_dns1));
        cJSON_AddItemToObject(cjson_temp, "dns1", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta2_dns2, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta2_dns2));
        cJSON_AddItemToObject(cjson_temp, "dns2", cJSON_CreateString(ip_str));
    }
    ...
    cJSON_AddItemToObject(cjson_temp, "enable", cJSON_CreateBool(iot_config_param.dev_info_t.wifi_mul_sta_en.sta3_en == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "ssid", cJSON_CreateString((char *)iot_config_param.dev_info_t.wifi_sta3_ssid));
    // cJSON_AddItemToObject(cjson_temp, "password", cJSON_CreateString((char *)iot_config_param.dev_info_t.wifi_sta3_password));
    cJSON_AddItemToObject(cjson_temp, "authmode", cJSON_CreateNumber(iot_config_param.dev_info_t.wifi_sta3_auth));
    cJSON_AddItemToObject(cjson_temp, "static_ip_enable", cJSON_CreateBool(iot_config_param.dev_info_t.wifi_mul_sta_en.sta3_static_ip_en == 1? true : false));
    if (1 == iot_config_param.dev_info_t.wifi_mul_sta_en.sta3_static_ip_en)
    {
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta3_ip, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta3_ip));
        cJSON_AddItemToObject(cjson_temp, "ip", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta3_gw, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta3_gw));
        cJSON_AddItemToObject(cjson_temp, "gateway", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta3_mask, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta3_mask));
        cJSON_AddItemToObject(cjson_temp, "netmask", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta3_dns1, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta3_dns1));
        cJSON_AddItemToObject(cjson_temp, "dns1", cJSON_CreateString(ip_str));
        // inet_ntoa(&iot_config_param.dev_info_t.wifi_sta3_dns2, ip_str, sizeof(ip_str));
        snprintf(ip_str, sizeof(ip_str), "%s", inet_ntoa(iot_config_param.dev_info_t.wifi_sta3_dns2));
        cJSON_AddItemToObject(cjson_temp, "dns2", cJSON_CreateString(ip_str));
    }
    ...
    cJSON_AddItemToObject(cjson_temp, "enable", cJSON_CreateBool(iot_config_param.dev_info_t.on_off.bit.ble_enable == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "adv_enable", cJSON_CreateBool(iot_config_param.dev_info_t.ble_protocol.adv_en == 1? true : false));
    // cJSON_AddItemToObject(cjson_temp, "password", cJSON_CreateString((char *)iot_config_param.dev_info_t.app_password));
    // HEX转换成字符串格式
    char *hex_str = hex_array_to_string(iot_config_param.dev_info_t.bles_adv_key, 16);
    ...
    cJSON_AddItemToObject(cjson_temp, "enable", cJSON_CreateBool(iot_config_param.dev_info_t.on_off.bit.Eth_enable == 1? true : false));
    ...
    cJSON_AddItemToObject(cjson_temp, "enable",
            cJSON_CreateBool(iot_config_param.dev_info_t.open_mqtt_enable.enable == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "crypt_enable",
            cJSON_CreateBool(iot_config_param.dev_info_t.open_mqtt_enable.crypt_en == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "ctrl_enable",
            cJSON_CreateBool(iot_config_param.dev_info_t.open_mqtt_enable.ctrl_en == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "report_enable",
            cJSON_CreateBool(iot_config_param.dev_info_t.open_mqtt_enable.report_en == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "url", cJSON_CreateString((char *)iot_config_param.dev_info_t.Net_Server_address));
    cJSON_AddItemToObject(cjson_temp, "report_cycle", cJSON_CreateNumber(iot_config_param.dev_info_t.open_mqtt_report_cycle));
    ...
    cJSON_AddItemToObject(cjson_temp, "enable",
            cJSON_CreateBool(iot_config_param.dev_info_t.modbus_tcp_enable.enable == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "crypt_enable",
            cJSON_CreateBool(iot_config_param.dev_info_t.modbus_tcp_enable.crypt_en == 1? true : false));
    cJSON_AddItemToObject(cjson_temp, "port", cJSON_CreateNumber(iot_config_param.dev_info_t.modbus_tcp_port));
...
#define WS_SVR_DEVICE_INFO_GET_RESP_TYPE   "device_info"
    ...
    //IOT
    if (reg_addr_rd.mod_reg11000_IOT_info.iot_sn > 0)
    {
        cjson_temp = cJSON_CreateObject();
        snprintf(str_tmp, sizeof(str_tmp), "%lld", reg_addr_rd.mod_reg11000_IOT_info.iot_sn);
        cJSON_AddStringToObject(cjson_temp, "sn", str_tmp);
        snprintf(str_tmp, sizeof(str_tmp), "V%lu", reg_addr_rd.mod_reg11000_IOT_info.software_ver);
        cJSON_AddStringToObject(cjson_temp, "version", str_tmp);
        cJSON_AddStringToObject(cjson_temp, "model", reg_addr_rd.mod_reg11000_IOT_info.iot_type);
        cJSON_AddItemToArray(cjson_device, cjson_temp);
    }

    if (reg_addr_rd.mod_reg01100_Inv_base.InvSN > 0)
    {
        cjson_temp = cJSON_CreateObject();
        snprintf(str_tmp, sizeof(str_tmp), "%lld", reg_addr_rd.mod_reg01100_Inv_base.InvSN);
        cJSON_AddStringToObject(cjson_temp, "sn", str_tmp);
        
        //有多少个固件版本就展示多少个
        memset(str_tmp, 0, sizeof(str_tmp));
        uint16_t str_offset = 0;
        for (int i = 0; i < reg_addr_rd_total.mod_reg01100_Inv_base.software_total; i++)
        {
            if (i < MD_SOFT_MAIN_CAP) {
                // 逆变器版本区域A
                str_offset += snprintf(str_tmp + str_offset, sizeof(str_tmp) - str_offset, "V%lu",
                                        reg_addr_rd_total.mod_reg01100_Inv_base.soft[i].version);
            } else {
                //逆变器版本区域B
                str_offset += snprintf(str_tmp + str_offset, sizeof(str_tmp) - str_offset, "V%lu",
                                        reg_addr_rd_total.mod_reg01100_Inv_base.soft_back[i - MD_SOFT_MAIN_CAP].version);
            } 

            // 添加逗号
            if (i < reg_addr_rd_total.mod_reg01100_Inv_base.software_total - 1) {
                str_offset += snprintf(str_tmp + str_offset, sizeof(str_tmp) - str_offset, ",");
            }
        }

        cJSON_AddStringToObject(cjson_temp, "version", str_tmp);
        cJSON_AddStringToObject(cjson_temp, "model", reg_addr_rd.mod_reg01100_Inv_base.InvType);
        cJSON_AddItemToArray(cjson_device, cjson_temp);
    }
...
#define WS_SVR_MD_LOGIN_RESP_TYPE   "loginRsp"
    ...
    if(iot_config_param.dev_info_t.app_password[0] != '\0')
    {
        int pwd_len = 0;
        // 计算APP密码长度
        for (pwd_len = 0; pwd_len < sizeof(iot_config_param.dev_info_t.app_password); pwd_len++)
        {
            if (iot_config_param.dev_info_t.app_password[pwd_len] == '\0')
            {
                break;
            }
        }
        ...
        if( strlen(password) != pwd_len
            || memcmp(iot_config_param.dev_info_t.app_password, password, strlen(password)) != 0)
        {
...
/**
 * @brief 处理扫描开始消息 scan_start
 * @param cjson_object 待处理cJSON对象消息
 * @param[out] resp_data 响应数据
 * @return 响应数据长度
 */
int16_t ws_svr_msg_scan_start_handler(cJSON *cjson_object, uint8_t **resp_data)
{
#define WS_SVR_SCAN_START_RESP_TYPE   "scanStartRsp"

    int16_t scan_mfg_id = -1;
    int16_t scan_type = 1;

    cJSON *cjson_type = cJSON_GetObjectItem(cjson_object, "type");
    cJSON *cjson_mfg_id = cJSON_GetObjectItem(cjson_object, "mfg_id");

    if (cjson_type == NULL || !cJSON_IsNumber(cjson_type))
    {
        ESP_LOGE(TAG, "scan_type is empty");
        return ws_server_msg_response(WS_SVR_SCAN_START_RESP_TYPE, 1, "ERROR : scan_type is empty", resp_data);
    }
    scan_type = cjson_type->valueint;

    if (cjson_mfg_id == NULL || !cJSON_IsNumber(cjson_mfg_id))
    {
        ESP_LOGE(TAG, "scan_mfg_id is empty");
        return ws_server_msg_response(WS_SVR_SCAN_START_RESP_TYPE, 2, "ERROR : scan_mfg_id is empty", resp_data);
    }
    scan_mfg_id = cjson_mfg_id->valueint;

    // Bluetti
    if(METER_MFG_ID_BLUETTI == scan_mfg_id)
    {
        // Bluetti设备通过WLCC长期在线，不需要扫描
        return ws_server_scan_device_progress((uint16_t)scan_type, (uint16_t)METER_MFG_ID_BLUETTI, resp_data);
    }
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE      
    else if (1 == scan_mfg_id)
    {
        reals.meter_scan_cmd.dev_type = 1;
        reals.meter_scan_cmd.mfg_id = METER_MFG_ID_SHELLY;
        reals.meter_scan_triger = 4;
        ESP_LOGI(TAG, "Start scanning Inverter with Shelly");
    }
    else if (2 == scan_mfg_id)
    {
        reals.meter_scan_cmd.dev_type = 1;
        reals.meter_scan_cmd.mfg_id = METER_MFG_ID_ECOTRACKER;
        reals.meter_scan_triger = 4;
        ESP_LOGI(TAG, "Start scanning Inverter with everHome");
    }
#endif
    else
    {
        return ws_server_msg_response(WS_SVR_SCAN_START_RESP_TYPE, 3, "Scan mfg_id is unknown!", resp_data);
    }
    
    return ws_server_msg_response(WS_SVR_SCAN_START_RESP_TYPE, 0, "Success", resp_data);
}

/**
 * @brief 处理绑定设备消息 bind_device
 * @param cjson_object 待处理cJSON对象消息
 * @param[out] resp_data 响应数据
 * @return 响应数据长度
 */
int16_t ws_svr_msg_bind_device_handler(cJSON *cjson_object, uint8_t **resp_data)
{
#define WS_SVR_BIND_DEVICE_RESP_TYPE   "bindDeviceRsp"

    int16_t bind_mfg_id = -1;
    int16_t device_type = 0;
    bool bind = true;

    cJSON *cjson_mfg_id = cJSON_GetObjectItem(cjson_object, "mfg_id");
    if (cjson_mfg_id == NULL || !cJSON_IsNumber(cjson_mfg_id))
    {
        ESP_LOGE(TAG, "bind mfg_id failed");
        return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 1, "Bind mfg_id failed", resp_data);
    }
    bind_mfg_id = cjson_mfg_id->valueint;

    cJSON *cjson_dev_type = cJSON_GetObjectItem(cjson_object, "device_type");
    if (cjson_dev_type == NULL || !cJSON_IsNumber(cjson_dev_type))
    {
        ESP_LOGE(TAG, "bind device_type failed");
        return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 2, "Bind device_type failed", resp_data);
    }
    device_type = cjson_dev_type->valueint;

    cJSON *cjson_bind = cJSON_GetObjectItem(cjson_object, "bind");
    if (cjson_bind == NULL || !cJSON_IsBool(cjson_bind))
    {
        ESP_LOGE(TAG, "bind field failed");
        return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 3, "Bind field is invalid", resp_data);
    }
    bind = cjson_bind->valueint;

    // Bluetti设备的SN是uint64_t的string类型，第三方的sn是dev_id
    cJSON *cjson_dev_sn = cJSON_GetObjectItem(cjson_object, "sn");
    if (cjson_dev_sn == NULL || !cJSON_IsString(cjson_dev_sn))
    {
        ESP_LOGE(TAG, "bind device_sn failed");
        return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind device_sn failed", resp_data);
    }

    // Bluetti
    if(METER_MFG_ID_BLUETTI == bind_mfg_id)
    {
        errno = 0;
        char *endptr = NULL;
        unsigned long long sn_tmp = strtoull(cjson_dev_sn->valuestring, &endptr, 10);
        if (errno != 0 || endptr == cjson_dev_sn->valuestring || *endptr != '\0')
        {
            ESP_LOGE(TAG, "bind device_sn parse failed, sn:%s", cjson_dev_sn->valuestring);
            return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind device_sn is invalid", resp_data);
        }

        uint64_t device_sn = (uint64_t)sn_tmp;
        if (device_sn == 0)
        {
            ESP_LOGE(TAG, "bind device_sn is invalid");
            return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind device_sn is invalid", resp_data);
        }

        POINT_BIND_INFO_WR bind_cmd = {0};
        bind_cmd.group_addr = bind ? 1 : 0;    // 0: 解绑，！= 0 绑定
        bind_cmd.Dev_Type = device_type;
        bind_cmd.SN_64 = device_sn;
        ESP_LOGW(TAG, "Bind device, bind:%d, mfg_id:%d, device_type:%d, sn:%llu", bind, bind_mfg_id, device_type, (unsigned long long)device_sn);

        if (SN_TYPE_METER == device_type)
        {
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE        
            meter_bluetti_bind_process(&bind_cmd);
#else
            return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind Meter is unsupported!", resp_data);
#endif
        }
        else if (SN_TYPE_S1 == device_type)
        {
#ifdef CONFIG_WLCC_PLUG_SUPPLY_ENABLE        
            plug_bind_process(&bind_cmd);
#else
            return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind Plug is unsupported!", resp_data);
#endif
        }
        else
        {
            return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind device_type is unknown!", resp_data);
        }
    }
#ifdef CONFIG_WLCC_METER_SUPPLY_ENABLE       
    else if (1 == bind_mfg_id)
    {
        meter_bind_info_t dev_info = {0};
        dev_info.set.bits.bind = bind ? 1 : 0;
        dev_info.dev_type = 1;
        dev_info.mfg_id = METER_MFG_ID_SHELLY;
        memcpy(dev_info.dev_id, cjson_dev_sn->valuestring, sizeof(dev_info.dev_id));
        meter_other_bind(&dev_info);

        ESP_LOGI(TAG, "Bind shelly device success");
    }
    else if (2 == bind_mfg_id)
    {
        meter_bind_info_t dev_info = {0};
        dev_info.set.bits.bind = bind ? 1 : 0;
        dev_info.dev_type = 1;
        dev_info.mfg_id = METER_MFG_ID_ECOTRACKER;
        memcpy(dev_info.dev_id, cjson_dev_sn->valuestring, sizeof(dev_info.dev_id));
        meter_other_bind(&dev_info);

        ESP_LOGI(TAG, "Bind ecotracker device success");
    }
#endif
    else
    {
        return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 4, "Bind mfg_id is unknown!", resp_data);
    }
    
    return ws_server_msg_response(WS_SVR_BIND_DEVICE_RESP_TYPE, 0, "Success", resp_data);
}
...
void ws_server_msg_init(void)
{
    ...
    ws_server_msg_register("scanDevices", ws_svr_msg_scan_start_handler);
    ws_server_msg_register("bindDevice", ws_svr_msg_bind_device_handler);
#endif
}
...
void web_server_task(void)
{
    ...
    static uint16_t update_dev_info_timer = 0;
    ws_msg_t recv_buffer = {NULL, 0, 0};

    if(ws_server_recv_queue && xQueueReceive(ws_server_recv_queue, &recv_buffer, 0)== pdTRUE)
    {
...
#if 0
    /* 更新BLUETTI设备列表信息 */
    if (++update_dev_info_timer >= 100)
    {
        update_dev_info_timer = 0;
        web_server_scan_device_report(0, METER_MFG_ID_BLUETTI);
    }
#endif
...
void web_server_scan_device_report(uint8_t type, uint8_t mfg_id)
{
    ...
    ret = ws_server_scan_device_progress((uint16_t)type, (uint16_t)mfg_id, &resp_data);
    if (0 < ret)
    {
        ESP_LOGI(TAG, "web server push scan device:%s", resp_data);
        // 发送响应数据
        for (size_t i = 0; i < WS_CLIENT_QUANTITY_ASTRICT; i++)
        {
            if (ws_client_list[i] > 0)
...
void web_server_start(void)
{
    ...
    /*创建接收队列*/
    ws_server_recv_queue = xQueueCreate(3 , sizeof(ws_msg_t));
    if (ws_server_recv_queue == NULL )
    {
        ESP_LOGE(TAG, "ws_server_recv_queue ERROR\r\n");
    }
