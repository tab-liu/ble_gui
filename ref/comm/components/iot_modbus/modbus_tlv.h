...
// Modbus TLV读取命令处理寄存器长度伪装使能
//#define MODBUS_TLV_READ_RTN_REG_LEN_FEIGN_ENABLE
...
typedef struct {
    ...
    /*读取命令预处理专用*/
    uint16_t real_len;     /* 本次读取中实际字节数 */
    uint16_t feign_len;    /* 本次读取中伪装字节数 */
} modbus_tlv_unit_t;
...
/**
 * 设备 X - 主动上报信息单元 y 的数据结构（按图示字段映射）
 *
 * 字节序/对齐：按需可加 __attribute__((packed))
 */
typedef struct {
    uint8_t  slave_addr;   /* 低字节: 从机地址 0..255 */
    uint16_t reg_addr;     /* 1..65535 (Modbus 寄存器地址) */
    uint16_t reg_num;      /* 寄存器数量 */
} modbus_tlv_announce_t;
